...
## 204. Zen from a maximized window: fixed at the cause, no restore round trip (unreleased, NOT verified on a real window)
...
Follow-up to §204 (Marien: entering is clean, but leaving a maximized Zen still shows the window behind for a moment): tao's un-fullscreen puts the frame styles back and maximizes again, painting an intermediate state. New command `zen_prepare_leave` runs just before it: with the window still fullscreen it sets WS_MAXIMIZE again and fills the work area, so the window already has its maximized geometry when tao switches the frame. `zenWindow.ts` remembers whether the window was maximized when Zen started and only then calls it (a failure is ignored; leaving still happens). Unverified natively.
