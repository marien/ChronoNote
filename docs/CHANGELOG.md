...
1. **Keyboard use of the Sync Review dialog.** It opens with focus on the **Sync** button (Enter accepts the review as it
   stands). Up/Down move between the "new meeting" checkboxes, wrapping; from the Sync button (or anywhere else) Down goes to
   the first and Up to the last. Tab still cycles through every control. The arrows are left alone inside the date field and
   the Leave/Discard/Move choice, which use them themselves.
2. **Cancelled, declined and forwarded meetings are removed meetings, named by the rest of their title.** `Canceled:`,
   `Cancelled:`, `Declined:`, `Followed:` and `Following:` (case-sensitive, at the very start; #74 had only the UK spelling
   of the first and `Following:`) no longer just vanish: the prefix is stripped and the rest is the meeting's real title, kept as
   a *removed* title. New `read_agenda_removed_for_date` returns them and `computeCalendarSync` takes them as a third
   argument: a section with such a title is reported as no longer on the calendar (empty ones under "Removed", ones with
   content under "No longer on the calendar" with Leave/Discard/Move) **wherever it sits in the note**. Before this, only sections
   inside the calendar block (from the first matching meeting to the end) could be found removed, so a cancelled meeting's
   section above that block was never mentioned. A live meeting with the same title on the same day wins, and a section already
   flagged `[CANCELED]` by an earlier "Leave" is left alone so it is never flagged twice. A day with only cancelled meetings
   still opens the review instead of saying "No meetings".
3. **"Placeholder" and "Confirmed" are dropped from a title.** `Placeholder: X`, `Confirmed - X` and `Placeholder -- X` are the meeting
   `X` (separator `:`, `-` or `--`, optional spaces). Without a separator, or with nothing after it, the word stays part of the
   title (`Confirmed attendees review`). A status-stamped and a plain entry with the same title and time are one meeting. It applies
   to the day's list, the "copy to next occurrence" search (`read_agenda_after`) and after a removed prefix (`Canceled: Placeholder -
   X` is the removed meeting `X`).

The desktop reads `.agenda.json` in Rust (`agenda.rs`, `classify_title`); the web app and the test mock share one TypeScript
implementation (`agendaTitles.ts`). Case-sensitive on purpose, like #74: fixed, syncer-generated text.
...
## 204. Zen from a maximized window: fixed at the cause, no restore round trip (v0.12.4)

Marien confirmed §203 covers the taskbar in v0.12.3 but the restore-then-fullscreen (and back) flickers; a hide/show variant was worse and is gone. Root cause found in tao 0.35.3 (`platform_impl/windows/window.rs` + `window_state.rs`): `set_fullscreen` drops the frame styles and moves the window to the monitor rectangle but leaves `WS_MAXIMIZE` set, and Windows keeps a `WS_MAXIMIZE` window inside the work area, hence the empty taskbar strip. Fix: new Rust command `zen_cover_monitor` (`src-tauri/src/zen_window.rs`, Windows only, no-op elsewhere; new direct dependency `windows` 0.61, already in the lock file) clears just the `WS_MAXIMIZE` style bit and moves the window onto `rcMonitor` in one `SetWindowPos`. `zenWindow.ts` now does `setFullscreen(true)` then, only if the window was maximized, `zen_cover_monitor`; leaving is a plain `setFullscreen(false)` and tao restores the maximized placement it saved. No unmaximize/maximize/hide/show, so the capabilities added in §203 for those are removed again. Tests: 7 unit tests (order of calls, no restore/hide, fresh state read per Zen, serialization, failures), mock handler for the command. Unverified natively; the assumption to check is that tao restores the window to maximized on leaving Zen.

Follow-up to §204 (Marien: entering is clean, but leaving a maximized Zen still shows the window behind for a moment): tao's un-fullscreen puts the frame styles back and maximizes again, painting an intermediate state. New command `zen_prepare_leave` runs just before it: with the window still fullscreen it sets WS_MAXIMIZE again and fills the work area, so the window already has its maximized geometry when tao switches the frame. `zenWindow.ts` remembers whether the window was maximized when Zen started and only then calls it (a failure is ignored; leaving still happens). Confirmed smooth on Windows 11 by Marien.

## 205. The caret after an open glyph with no following text no longer hugs the shrunk checkbox (#93)

Marien: placing the caret right after `# ` with nothing typed yet made it look glued to the checkbox rather than sitting past the (invisible) trailing space, tempting people into typing a redundant extra space. Root cause: §84's `transform: scale(0.85)` shrinks the checkbox glyphs (☐ ☑ ☒) on the exact `span` CodeMirror measures to place the caret right after an atomic replace-decoration widget — its own `getBoundingClientRect()` shrinks along with the drawn mark, so the caret rendered at the *visually shrunk* glyph's edge instead of the full 2ch cell's edge. Fixed by moving the `scale()` off that outer span onto a newly nested `.glyph-ink` child (`glyphs.ts`'s `InlineGlyphWidget.toDOM()`); `translateY` (vertical-only, §87's baseline fix) stays on the outer span since it doesn't affect the box CodeMirror measures horizontally. Verified directly against DOM/`getBoundingClientRect()` data in a running dev instance (not just the test suite) before and after, confirming the caret moves from the shrunk glyph's edge to the true cell edge. New Playwright regression test (`glyph-layout.spec.ts`) compares the caret's x-position after typing `# ` against an independent reference (two plain, untransformed characters elsewhere) rather than against the glyph's own box, which would be circular — confirmed it fails against the unfixed code (~3px off) and passes against the fix. `svelte-check` 258/0, Vitest 576/576 (unchanged — see §206 for the +2), Playwright 375/375 (+1; two pre-existing spinner-timing tests flake under full-suite parallel load, confirmed unrelated by passing in isolation), `cargo test` 150/150 (unchanged, pure frontend).

## 206. A repeat calendar sync no longer re-flags an already-cancelled meeting (#92)

Marien: leaving a cancelled meeting's section via Sync Review's "Leave it" correctly prefixes it `[CANCELED] …` and moves it to the end of the note, but running Sync again while the meeting is still cancelled added a *second* `[CANCELED]` prefix — and asked that a meeting reappearing on the calendar be treated as "uncancelled" (the imported content used, not a doubly-stale marker). Root cause in `calendarReconcile.ts`'s `computeCalendarSync`: the existing "already flagged, don't flag twice" guard (`isRemovedSection`) only covers the path for a title `.agenda.json` currently reports as declined/cancelled — it never protected the *general* "this calendar-block section has no current agenda match, surface it for review" path (`blockSections.forEach` → `noteRemoved`), which fires for **any** unmatched section physically inside the block. Since `flagRemovedSection` appends the flagged section at the tail — after the live meetings, i.e. *inside* the block, not before it, unlike every existing `#78` test's fixture ordering — that section was still unmatched on the next sync and got silently re-reported and re-flagged. Two `#93`-adjacent test cases: the existing `#78` regression test's fixture had the cancelled section positioned *before* the matched one, which never reached the buggy code path at all — a new describe block builds the fixture in `flagRemovedSection`'s own real append order instead. Fixed with a shared `isCancelledHeader()`/`stripCancelledPrefix()` pair: an already-`[CANCELED]`-prefixed section still unmatched is now carried through into the output unchanged rather than re-reported (no second flag, ever); one that *does* match a live agenda title has its prefix stripped and its underline recomputed for the now-shorter title, un-cancelling it. Both new tests confirmed to fail against the unfixed code before the fix landed. `svelte-check` 258/0, Vitest 576/576 (+2), Playwright 375/375 (unchanged — pure reconciliation logic, no UI-visible new interaction to cover beyond the existing Sync Review flow), `cargo test` 150/150 (unchanged, pure frontend).

## 207. Post-release cleanup for v0.12.5: CI build-guard/test flake, and stale website icon caching

Found working the release, not chat feedback. Two unrelated problems surfaced right after v0.12.5 shipped, both fixed directly on `main` (no version bump — neither touches the desktop app's own build):

**CI red on the release commit.** `node_modules` had drifted from `package-lock.json` (`vitest` still 5.0.0 against a lockfile already requiring 5.0.1 — an `npm install` that was never re-run after an earlier session's dependency-bump pull). The demo/webapp bundles built from that stale state no longer matched what CI's own clean `npm ci` produces, tripping the `build-guard` job's `git diff --exit-code -- website/webapp/` check. Fixed with a clean `npm install` and a full rebuild. Separately, the new `#93` Playwright test (§205) flaked in CI — root-caused via a throwaway diagnostic PR (`debug/93-linux-diag`, closed without merging) that logged the real DOM geometry from Linux CI: the fix's transform separation is correct there too (`outerTransform: matrix(1,0,0,1,0,0.65)`, `innerTransform: matrix(0.85,...)`, exactly as designed), but the cursor overlay's position can read stale for one frame right after the last keystroke — a genuine timing race, not a font/platform difference. Now polls (`expect.poll`) instead of reading once.

**Website/webapp icon caching.** Marien noticed the new icon (§ redesign preceding v0.12.5) wasn't showing up and didn't want to have to clear his own browser cache to see it. Two separate causes: (1) the marketing website's own favicons (`website/assets/favicon.ico`/`favicon-32.png`/`apple-touch-icon.png`) are a completely separate copy from the app/webapp icons and were never touched by the redesign commit at all — regenerated from the same already-updated source (`src-tauri/icons/{icon.ico,32x32.png,128x128@2x.png}`, already the right sizes) and cache-busted with a `?v=0.12.5` query param on all three `<link>` tags, since browsers cache favicons unusually aggressively. (2) the web app's own icon was current, but its service worker (`webapp-src/public/sw.js`) cached it under a hand-maintained `CACHE_NAME` literal (`"...-v2"`) that nobody remembered to bump for this release, so `activate`'s old-cache cleanup never saw a name change and never invalidated the stale entry. Fixed generally rather than just this once: a new `injectSwVersion` Vite plugin (`vite.webapp.config.ts`, `closeBundle` hook) patches the built `sw.js`'s `__WEBAPP_VERSION__` placeholder with the real `package.json` version, so every future release — including one that only touches an icon and never changes `sw.js`'s own logic — still ships a genuinely new cache generation. Also added `{cache: "reload"}` to the precache fetches, so a long-lived `Cache-Control` on the static host can't hand back stale bytes into even a brand-new cache bucket. Verified live end-to-end on the deployed site post-promotion: the marketing site serves the new favicon bytes (sizes match exactly), and the web app's Cache Storage holds exactly one cache, `chrononote-webapp-shell-0.12.5`.

`website/README.md`'s Publishing section gained a note for the next real icon redesign: regenerate the three website favicons and bump their `?v=` by hand (the manual step that was missed this time); the web app side needs no equivalent reminder anymore. `svelte-check` 258/0, Vitest 576/576, Playwright 377/377, `cargo test` 150/150 — all re-verified, and CI confirmed green on the actual pushed commit (`b7cd195`) before promoting `website-live`.

**Follow-up (same day):** Marien pointed out the icon at the top of the website itself was still the old one — a third place the earlier pass missed. `website/index.html`'s and `guide.html`'s header brand mark next to "ChronoNote" is a hand-coded inline SVG (a small page-with-three-lines outline), not a reference to any icon file at all, so none of the favicon/service-worker fixes above could have touched it. Redrawn to the new "daily agenda" concept — rounded page, header rule, twin binder pins, checkmark — scaled down from `docs/design/icon-A-master.svg` into the same 24x24 line-icon style (`stroke="currentColor"`, `stroke-width="1.75"`) already used by every other icon on the page. No caching layer involved (plain inline markup in the page HTML), so this went live immediately on the next `website-live` promotion with no cache-busting needed. Confirmed by grepping the whole `website/` tree for the old icon's exact path data to make sure no fourth copy was hiding somewhere.

**Second follow-up (same day):** Marien reported the web app's own icon still hadn't updated for him, even after the service-worker cache-name fix above. Real gap, not a caching fluke: the SW fix only closed the Cache Storage layer — `webapp-src/index.html`'s `<link rel="icon">`/`apple-touch-icon>` hrefs and `manifest.webmanifest`'s `icons[].src` still referenced the icon files with no cache-busting query param of their own, and browsers cache favicons and PWA-manifest icons through their own aggressive, largely SW-independent mechanisms. Fixed by extending the same version-injection plugin (renamed `injectVersionPlaceholders`, since it now patches more than `sw.js`) to also patch a `__WEBAPP_VERSION__` placeholder into the built `index.html` and `manifest.webmanifest`, and updated `sw.js`'s own precache list to reference the same versioned icon URLs so they're cached under the exact key the page actually requests. Verified live: every icon reference on `app.chrononote.mariendegelder.nl` now carries `?v=0.12.5`. `svelte-check` 258/0, Vitest 576/576 re-verified.

## 208. Android app dropped (v0.13.0)

Marien: the PWA-installable web app now covers phone use as well as the native Android app did, so the sideloaded-APK-only Android target (no store listing, ever) is no longer worth the ongoing Android SDK/NDK toolchain maintenance, manual release signing, and pinned-older-Rust cross-compile workaround (§181–§186) it required. Asked for a full removal plan first (investigated across Rust, frontend, build tooling, docs and the website), then to execute it as one pass/one commit followed by a dedicated release.

**Headline finding: nothing needed porting.** The two capabilities that looked structurally Android-specific both already had independent, already-shipped web app equivalents: OneDrive sync (§187–§188's own TypeScript engine, not the Rust one) and OAuth sign-in (the web app's full-page PKCE redirect works the same whether the page is a browser tab or an installed PWA window, no `chrononote://auth` deep link needed). The touch UI (Mobile Accessory Bar, tabs drawer) was already plain Svelte gated on `pointer: coarse`, already dogfooded on the mobile web app (§189). Two Android-only mechanisms had no PWA equivalent to build because the problem itself doesn't exist for a PWA — pure removal: OS-level backup exclusion for the OneDrive tokens, and the deep link itself.

**Removed.** Every `#[cfg(target_os = "android")]` branch in `onedrive/auth.rs` and `onedrive/sync.rs` (un-gating the desktop variants in their place) and the Android backup-exclusion test; the `tauri-plugin-deep-link` registration, `wire_onedrive_deep_link`, and the Android setup block in `lib.rs`, plus the Cargo dependency and the `tauri.conf.json` plugin block; `src-tauri/android-overrides/` in full; `src/lib/androidChrome.ts` and its `--inset-*` CSS custom properties (every consumer already falls back to `env(safe-area-inset-*)` directly, which a PWA needs anyway); `scripts/android-release.sh` and `Launch_ChronoNote_Emulator.bat`; the `ChronoNote-android-build` git worktree. `backendKind` narrows from `"desktop" | "demo" | "web" | "android"` to `"desktop" | "demo" | "web"` — every gate on it simplified, in most cases deleting the `"android"` half of an existing condition (Zen mode is no longer excluded on any target; the sync telemetry popover and Sync conflicts drawer are now a plain web app feature).

**Two corrections made along the way, not assumed from the plan.** `OneDriveLoginResult.pending` was first deleted on the assumption it existed only for the Android deep-link's "returns immediately, real result arrives later" flow — `npm run check` caught 7 real errors pointing at `src/lib/webapp/webOneDriveSync.ts`, which returns `pending: true` for its own redirect-based flow; restored, with a corrected doc comment. Tracing `OneDriveFolderPickerModal.svelte`'s `finalizeFolderSelection` branch-by-branch also found that `keepNotesDir`/`SwitchOptions` (`directory.ts`) — described as "shared with web" — was provably unreachable for web even before this change, since the web branch always won first regardless of `keepNotesDir`'s value; deleted outright (`directory.ts`, the folder-picker modal, and the one `controller.test.ts` test that exercised it) rather than just reworded.

**Docs and website.** `docs/spec.md`: §7.6 "Android App & OneDrive Sync" deleted in full; "One Frontend, Four Build Targets" → "Three"; the build-target table, `backendKind` type, Zen-mode and telemetry-popover restrictions, and the Sync conflicts/OneDrive-sync product descriptions updated to describe the web app plainly instead of "Android (and the web app)". `docs/design/android-onedrive-roadmap.md` marked superseded (kept for design history, matching the parked M365 roadmap's precedent) rather than deleted; its `docs/design/README.md` index entry updated to match. `docs/AGENT_GUIDELINES.md`'s "four targets" framing corrected to three. `website/index.html`'s and `guide.html`'s `#android` sections replaced with a short "install the web app to your home screen" section and PWA install steps (Chrome's "Add to Home screen", Safari's "Add to Home Screen"); the OneDrive-connect and conflict-resolution steps in the old Android section carried over verbatim, since they were never actually Android-specific instructions to begin with. `docs/CHANGELOG.md` and past GitHub releases (including the Android APK assets already attached to them) are untouched, per this project's established convention of never rewriting history.

**Left as-is, flagged rather than expanded into scope:** the desktop build still compiles the native Rust OneDrive engine (`onedrive/*.rs`, its `cargo test` coverage intact), but no desktop UI has ever exposed it — desktop users point their notes folder at a real, OS-synced OneDrive folder instead. That was already true before this change and is unrelated to dropping Android; not touched here.

Verification: `svelte-check` 257/0 (258 minus `androidChrome.ts`), Vitest 575/575 (576 minus the deleted `keepNotesDir` test), `cargo test` 149/149 (150 minus the deleted Android backup test), Playwright 375/375 (one pre-existing spinner-timing flake under parallel load, confirmed unrelated by isolated rerun), `npm run tauri build` produces both installers cleanly with the deep-link plugin gone.

## 209. A selected tab's top border matches its own date color (#96), and the Action Drawer can show just the notes you haven't reopened

**#96:** the top bar's `.tab.active` top border (`box-shadow: inset 0 2px 0 var(--tab-active-border)`) was always the "today" accent color, regardless of whether the *selected* tab was past, today, or future-dated — even though §68 already gives each tab's own icon a distinct color for exactly that distinction (dimmed for past, accent for today, `--state-ok` green for future). Selecting a past or future tab left its top border reading as "today" anyway. Fixed with two new rules in `app.css`, `.tab.active.past` (`color-mix(in srgb, var(--muted) 45%, transparent)`, matching the icon's own dimmed treatment) and `.tab.active.future` (`var(--state-ok)`, matching the icon's green) — `.tab.active.today` needed no override, since the existing default already matches. New Playwright test in `tab-archetypes.spec.ts` selects each of the three states in turn and asserts the computed `box-shadow` differs across all three.

**Action Drawer "Other Notes" scope.** Marien: today's two-way "Open Tabs" / "All Files" scope has no way to see *just* the notes you haven't reopened — "All Files" mixes them back in with whatever's already open. New `buildActionSnapshotOtherNotes()` (`actions.ts`) filters `buildActionSnapshotAllFiles()`'s own result down to items with no open tab (`item.tabId === undefined`) rather than a separate disk read, so the two scopes can never disagree about what counts as "open." `ActionDrawerModal.svelte`'s `Segmented` gains a third option, "Other Notes", between the existing two. New Playwright test seeds one note in an open tab and one closed on disk, confirming the new scope shows only the closed one.

**Naming collision, flagged and left alone by request.** Raised in passing: "Open Tabs" (a scope) and "Only Open" (an action-state filter) read as related but mean different things, and now sit right next to each other with a third "Other Notes" option added. Presented three options (rename the scope labels, rename the state checkbox, or just add clarifying tooltips) — Marien picked the lowest-effort one: both sets of labels stay exactly as they are, and `Segmented` gained an optional per-option `title` (native hover tooltip) now used for all three scope options plus the "Only Open" toggle's own label, explaining each in a few words without touching any visible microcopy or the one existing `docs/spec.md` reference to "Only Open".

Verification: `svelte-check` 257/0, Playwright 377/377 (+2).

## 210. The web app's PWA icon on Android: too large, not enough padding

Marien, installing the web app to an Android home screen: "the icon looks slightly too large, with not enough space around it. Probably my phone is doing some clipping or resizing." Root cause: `manifest.webmanifest`'s two icon entries were marked `"purpose": "any maskable"` — the same image serving both purposes — but the underlying PNGs (`icon-256.png`/`icon-512.png`, generated from `docs/design/icon-A-master.svg`) are a normal edge-to-edge icon, with the glyph content filling roughly 75% of the canvas height, well beyond the ~60-66% "safe zone" a maskable icon needs to survive an OS-applied circular/squircle mask without clipping. Android was applying its own adaptive-icon mask on top of art that was never designed with a safe margin for one, cropping straight into the calendar glyph's corners and pins — exactly the "too large, clipped" look reported.

Fixed by generating genuinely separate maskable variants rather than reusing the existing files: a new SVG (rasterized via headless Edge, no new build dependency) drops the master icon's own baked-in rounded-square background entirely — a maskable icon should be a flat, edge-to-edge fill in the OS's hands, not pre-rounded — and re-centers the bare glyph strokes at 80% of their original scale (anchored on the glyph's own bounding-box center, not the canvas center, since the two differ slightly), landing the content at ~60% of canvas height. Verified by rendering the result through a 100%-diameter circle clip-path (the harshest mask any launcher would realistically apply) before shipping it — comfortable padding on every side. New `icons/icon-{256,512}-maskable.png` alongside the existing (untouched) `icon-{256,512}.png`; `manifest.webmanifest`'s four icon entries now split `"purpose": "any"` (the original files, for anything that shows the icon as-is) from `"purpose": "maskable"` (the new padded files), and `sw.js`'s `PRECACHE_URLS` picked up the two new versioned URLs alongside the existing pair. iOS's `apple-touch-icon` links are untouched — that mechanism has no `purpose` concept and applies its own much more forgiving corner-rounding directly to the original art, which was never the reported problem.

Verification: `npm run build:webapp` produces a manifest with all four entries correctly split and version-stamped; visual check of both new PNGs directly and through a simulated circular mask.

## 211. Section History: browse full occurrences in context, carry lines forward (redesign)

Marien, after living with the §150-era drawer: "What I realize when using it, is that I don't use the action list itself, I use it to move between occurrences, and check the actual notes, including actions, so I can see actions and follow-ups in context." Proposed a redesign (`docs/design/section-history-browse-and-carry-forward-roadmap.md`), got three decisions confirmed, then built it the same day.

**What changed.** The drawer's flat, deduped, cross-occurrence action list — and the "Only Open" toggle that filtered it — are gone. The left list is now just occurrence rows (one per dated note with the section, past/present/future, an empty one tagged "no content yet"); selecting one shows its *entire* body in the right pane, glyph-rendered exactly like the editor (the old "Previous occurrence" pane's treatment, now applied to every occurrence instead of just the nearest one). Clicking a line — Shift+click to extend a range — selects it for "take it over": a destination bar appears with button(s) computed once when the drawer opened, from the note it was opened from, never re-decided per occurrence browsed: a single "Insert here" when that note is today, in the future, or a scratchpad; "→ Today" and (if one exists) "→ Next occurrence" when it's in the past, found via the same search `Ctrl/Cmd+Shift+.` already uses (now extracted into `findNextOccurrenceTarget`, shared by both). A single line shaped `prose => action` also offers "Whole line" vs. "Action only" (just the action, dropping the prose). Taking it over defers the source (a deferred line taken over is re-adopted as a fresh open action, matching the old `historyInsertText`'s rewrite) and inserts into the target section, creating it if needed; the drawer stays open afterward (re-scanning disk) so several items can be pulled across one browsing session. Browsing the note the drawer was opened from offers no take-over — nothing to carry it to.

**Implementation.** `commitCopyForward` (`copyForward.ts`) generalized from "always the active tab, always a dated-file target" to a `sourceFilename` (read/written through an open tab if one exists, straight to/from disk otherwise) and a `CopyTarget` (`{kind: "tab"}` or `{kind: "date"}`), with an optional `insertLinesOverride` for the "Action only" case — the verbatim #66 behavior is unchanged, just re-expressed through the more general shape. New `findNextOccurrenceTarget` extracts #66's calendar-then-disk search into a function both `copySelectionToNextOccurrence` and Section History's own `computeHistoryDestinations` call. New `carryHistorySelectionForward` is History's own entry point. `history.ts` lost `isOpenHistoryAction`/`historyActionsForLine`/`normalizeActionText`/`findPreviousSectionOccurrence`/`historyInsertText`/`importHistoricalItem` (the flat-list machinery) and gained `computeHistoryDestinations`, `historyActionOnlyText`, `historyTakeOverLines`, and a new `reopenDeferredAction` in `tokens.ts` for the re-adoption rewrite. `HistoryItem`/`PreviousSectionOccurrence` types removed; new `HistoryDestination` type and `historyDestinations`/`historyOpenedFromTabId` stores added, `historyItems`/`historyShowOnlyOpen`/`historyPreviousOccurrence` removed. `HistoryModal.svelte` rewritten around an occurrence list driving a full-body detail pane with line-range selection.

**Decisions confirmed before building (see the design doc's §8):** drop the flat list/"Only Open" toggle outright, no secondary mode kept; a take-over's source note is updated on disk directly when it isn't an open tab, never opening one just to defer a line; resolved (dimmed) lines stay selectable, since re-adopting a done/deferred item as fresh open work is a legitimate case.

Verification: `svelte-check` 257/0; `controller.test.ts` 173/173 in isolation (tests renamed/consolidated along with the functions they cover — `historyInsertText`'s became `historyTakeOverLines`'s, `isOpenHistoryAction`/`historyActionsForLine`/`findPreviousSectionOccurrence`'s removed with their functions, new tests added for `computeHistoryDestinations` via `openMeetingHistory` and for `carryHistorySelectionForward` directly); `search-and-history.spec.ts` rewritten for the new behavior (occurrence browsing, line selection, all three destination shapes, the own-occurrence guard) and passing in isolation. A genuinely heavily-loaded machine this session made a trustworthy full-suite run hard to get (repeated truncated/contended output on both Vitest and Playwright, unrelated to this change); every file touched here was separately confirmed clean on its own. The one `#62` spinner-timing test in this file joins the two already-documented ones as flaking under heavy parallel load — confirmed unrelated to this change by passing reliably in isolation and by passing with a longer artificial delay at the same 1000ms margin the pre-existing ones use.

## 212. Section History: occurrences as a compact tab strip, full keyboard/mouse line navigation; a modal-focus-escape bug fixed generally

Marien, after trying §211's redesign: "There is a lot of empty space now... I am probably mainly interested in the last couple of occurrences, which would also [be] in a top bar instead of a sidebar, very similar to how the main topbar with tabs works... And show in the tab title with the colored dots from the agenda." Plus a full keyboard/mouse navigation spec (arrows move the line selection, Shift+arrows extend it, Left/Right cycle occurrences and wrap at the ends; click selects one line, click-drag selects a range) and a bug: "the Section History drawer needs to keep the focus until it is closed... CTRL+TAB allowed me to switch between the main tabs, and then I could move up and down with the arrows."

**Layout.** The vertical, one-row-per-date sidebar list is gone. Occurrences are now a compact horizontal strip above the note body (`.history-occ-strip`), modeled on the main window's own tab strip but smaller — horizontally scrollable, no visible scrollbar, keyboard Left/Right and `scrollOccIntoView` keep the selected one reachable regardless of count. Each tab carries a small dot mirroring the date picker's own 3-tier completion heat (`computeDayHeat`, reused via a new `occurrenceHeat(occ)` in `history.ts` scoped to the section's own lines instead of the whole file) — amber for open actions, green for all resolved, muted for a note with no actions at all; no dot at all means the section is genuinely empty yet (`.history-occ-tab.empty`). `.history-body`/`.history-detail` collapsed from a two-column (`.history-main` + `.history-detail`) layout into a single column, since the strip no longer needs its own width-competing lane — the note body now gets essentially the whole card. The old mobile-only "Occurrences / Preview" tab switcher (`.history-mobile-tabs`) is gone entirely: strip-above-body fits any width without needing to hide one for the other.

**Keyboard and mouse.** `HistoryModal.svelte`'s selection model is now a text-editor-style anchor+focus pair (`selAnchor`/`focusLineIdx`), not a fixed-anchor Shift+click extend: Up/Down move a single-line selection; Shift+Up/Down grow *or shrink* the range from wherever the anchor already is; Left/Right cycle the browsed occurrence with wraparound (`wrapIndex`, the same helper the main tab strip's own Ctrl+Tab uses). Mouse: a plain click (`mousedown`) selects one line and arms a drag; each line the pointer enters while the button is still down (`mouseenter`) extends the same selection, ended by a window-level `mouseup`. The keydown handler moved off a template `on:keydown` (which needs an interactive ARIA role svelte-check rightly flags on a plain `<div>`) onto a `document`-level listener scoped to the modal's own mount/unmount — the same "listen where the keys land, not where one div's focus happens to be" approach `focusTrap` already uses for Tab.

**The focus-escape bug was real and general, not History-specific.** `App.svelte`'s global keydown handler dispatched its entire shortcut table (`newScratchpad`, `cycleTab`/Ctrl+Tab, `closeTab`, `openHistory`, `openSettings`, …) unconditionally — only `commandPalette` and `findInNote` had their own explicit `get(modal) !== "none")` bail-out. Any other modal open, and Ctrl+Tab (or Ctrl+N, or Ctrl+,, or any entry in the table) still reached the app underneath, which is how Marien could cycle the main tab strip and then move the *editor's* cursor with arrow keys while History sat open on top. Fixed with one added gate ahead of the whole dispatch loop, matching the pattern the other two already established — closes the same hole for every shortcut in the table, not just the two combos already guarded and not just for History.

**Implementation notes.** `HistoryModal.svelte` no longer uses the shared `virtualList` row-model machinery beyond `clampIndex`/`wrapIndex` — the occurrence strip is plain unvirtualized buttons (cheap even at a few hundred), so `withTops`/`stackHeight`/`visibleWindow`/`scrollToShow`/`MODAL_ITEM_ROW_HEIGHT` dropped out along with the vertical list they existed for. The now-orphaned `.modal-group-header.selectable`/`.selected` CSS (§150, only ever consumed by History's old date-header rows per its own comment) removed from `app.css` as dead code.

**A real test-harness bug found and fixed along the way, unrelated to Section History itself:** `seedApp()` (the shared Playwright helper) never cleared `sessionStorage` before navigating, but `installMockTauri` deliberately prefers whatever's in `sessionStorage` over a fresh seed on reload (so a real `page.reload()` can test "…survives a restart"). A test calling `seedApp()` a second time with a different seed — an established pattern in this exact spec file — silently got whichever seed happened to win a race against the first page's own mutating-command timing, not reliably the new one. Caught rewriting `mobile-ergonomics.spec.ts`'s History test (a seeded second note was sometimes just missing); fixed at the root in `seedApp()` itself (clears `sessionStorage` right before navigating) rather than worked around per-test, since every other double-seed test in the suite had the same latent exposure. Re-verified the four specs that actually exercise the real "survives a reload" path (`settings.spec.ts`, `update-check.spec.ts`, `word-wrap.spec.ts`, `merged-titlebar.spec.ts`) still pass — that mechanism itself (`page.reload()` with no re-seed) is untouched by the fix.

Verification: `svelte-check` 257/0; Vitest full suite 582/582 (+5, `occurrenceHeat`); Playwright, run targeted rather than full-suite given this session's own repeated environment contention (see §211's note on the same issue) — `search-and-history.spec.ts` 37/37, `mobile-ergonomics.spec.ts` 6/6, plus a cross-section of every other spec exercising modals/shortcuts (`command-palette`, `drawers`, `mac-shortcuts`, `modal-a11y`, `navigation`, `action-drawer`, `tabs-lifecycle`, `modal-system`, `topbar-collapse`, `tab-scroll-arrows`, `smoke`, `status-bar`) all green in isolation, single-worker.

## 213. Section History: chronological strip order, per-date colors, clearer take-over labels, a fixed-height modal

More chat feedback on §212, from actually using the new strip. Five separate asks, all in one pass:

**Chronological order, newest to the right.** `buildOccurrences` (`history.ts`) sorted descending (newest first); the strip rendered that order left-to-right, so the strip read newest-to-oldest — backwards from the main tab strip's own earliest-to-latest convention it's modeled on. Now sorts ascending; the drawer still opens focused on whichever date it was opened from (`onMount`'s `findIndex`, unaffected by which direction the array runs), it just may now be at either end of the strip rather than always the left one.

**Per-date coloring, matching the main tab strip.** `occDateClass(occ, today)` (mirroring `TopBar.svelte`'s own `tabDateClass`, `$currentDateISO` for the same midnight-rollover reason) classes each tab `past`/`today`/`future` regardless of selection — dims a past date's text, accents today's, greens a future one — and the *selected* tab's own underline additionally follows the same class (`.history-occ-tab.active.past`/`.active.future`, `.active` alone already matching today's default accent, the same #96 precedent applied here).

**No filename, double-click to jump.** Marien: the tab already names the date, so repeating the full filename in the detail pane (and a separate "Open file" button to act on it) was redundant. Both removed; double-clicking an occurrence tab calls `jumpToHistoryLine(occ)` directly and closes the drawer, working regardless of whether that tab was already the one selected.

**Take-over buttons that say what they do.** Marien: "Insert here" (and "→ Today") didn't say *where* "here" was, and it wasn't obvious the action appends to the end of the section without deleting the source. `computeHistoryDestinations` (`history.ts`) now builds a label from the actual destination — "Add to today", `Add to <date>` (a future note), `Add to "<scratchpad name>"`, or `Add to next occurrence (<date>)` — and the take-over bar always shows a one-line caption ("Moves the selected line(s) to the end of that section — marked forwarded (») in this occurrence, not deleted.") next to the buttons. Also asked for a keyboard path for the whole flow ("move to a line with the arrows, select multiple lines if needed, press shortcut to insert into section on main tab"): new `Shift+Enter` triggers the *first* destination `$historyDestinations` offers — always the nearest one — with the footer hint naming exactly which one it is, so it's never a guess.

**A fixed-height modal.** Marien: "Section history height needs to take up ~80% of the screen space. Don't change the height when switching between dates." `.history-modal-card`'s `max-height: 80vh` (§154/§212) let the card shrink to fit a short occurrence and grow for a long one — switching Left/Right visibly resized the whole modal. Changed to a plain `height: 80vh`, constant regardless of content; a short occurrence just leaves empty space in `.hp-context` instead of shrinking the card down to meet it.

Verification: `svelte-check` 257/0; `controller.test.ts` 177/177 (destination-label and occurrence-order assertions updated for the new label text and ascending sort); `search-and-history.spec.ts` 30/30 (including 5 new tests: strip order, focus-on-open explicitly asserted, the past/today/future colored line, `Shift+Enter`, and constant height across a short vs. an 80-line occurrence), `mobile-ergonomics.spec.ts` 6/6 — both single-worker.

## 214. Smaller modal title bars, and a real bug in Section History's take-over destinations

Two separate items from chat.

**Modal title bars.** Marien: "Why are the titles bars of the modal windows so large? They could be smaller." `.modal-input-wrap` (the shared title/search row every modal uses) had `padding: 12px` on every side around a 32×32px `.modal-close-btn`, putting the row at ~56px tall regardless of what a given modal actually needed. Tightened to `padding: 8px 12px` and a 26×26px close button (its `pointer: coarse` invisible 44×44px hit-area is untouched, so touch targets aren't affected) — measured at ~42px now, about a 25% reduction, applied uniformly since every modal shares this one class.

**Section History take-over bug, found by Marien testing the new destinations.** Reported: "I opened the drawer on a date in the past. When I select lines on that date, I cannot add them to today. When I select lines on today, I can add them to today, which does not copy them but does mark an open action as deferred." Both symptoms traced to the same wrong rule, `isOwnOccurrence`: it disabled *all* take-over whenever the browsed occurrence was the one History was opened from — a blanket check against the wrong thing.

- **Symptom 1 (nothing offered from the opened-from note):** wrong when opened from the *past* — "Today" and "Next occurrence" are different files entirely from the opened-from note, so there's something real to forward to. The old rule blocked it anyway, purely because you happened to be looking at the note you started from.
- **Symptom 2 (the silent data loss):** the rule never caught the *mirror* case — browsing *today's own* occurrence while "Today" is offered as a destination is exactly as self-referential (source file === target file), but `isOwnOccurrence` only ever compared against the opened-from filename, not each destination's own target. `commitCopyForward` (`copyForward.ts`) then ran two independent read-modify-write passes — insert into the target section, defer the source range — against the *same* file, each starting from the same pre-edit content. Whichever write landed last won outright, silently discarding the other's edit. The reported symptom (insertion vanishes, only the deferred-line edit survives) is exactly what that race produces when the defer-write happens to go last, which it always does in `commitCopyForward`'s fixed call order.

**Fix, in two layers.** (1) `HistoryModal.svelte` replaces `isOwnOccurrence` with `usableDestinations` — `$historyDestinations` filtered per occurrence, dropping any destination whose actual target file (`destinationFilename`: `openedFromFilename` for "here", `` `${date}.txt` `` for "today"/"next") equals the *browsed* occurrence's own filename. This is used everywhere the old flag was: the take-over bar, the "nothing to carry this over to" fallback message (reworded, since it's no longer only true for the opened-from note), the footer's `Shift+Enter` hint, and the shortcut itself. (2) `commitCopyForward` hardened as defense-in-depth: when the resolved target filename equals the source filename, it now does one combined edit (insert into the section, then defer the source range *within that same already-updated content* — the insertion always lands after the source range, so the original line indices stay valid) and one write, instead of two independent passes that can race on any path that reaches this function, not just Section History's.

**The full expected-behavior matrix**, worked through and now covered by tests:
- **Opened from today, a future date, or a scratchpad:** the only destination is always "here" — wherever you started. Offered for any occurrence you browse *except* that exact one (nothing to carry a line to when you're already looking at it). Unaffected by whether other sections exist elsewhere — unchanged from before this fix.
- **Opened from a past date, a section already exists today:** `findNextOccurrenceTarget`'s search stops at the first match after the anchor, which is today — so "Next occurrence" is *never* computed at all here, "Today" alone covers it, regardless of whether a further-future section also exists. Browsing the opened-from note or any other past/future occurrence offers "Add to today"; browsing today's own occurrence offers nothing (the only destination there ever was is itself).
- **Opened from a past date, no section today but one exists in the future:** "Today" is always offered (it can create the section); "Next occurrence" is also offered, pointing at that future date. Browsing the future occurrence hides "Next occurrence" (self-referential) but keeps "Today"; today itself never appears in the strip at all here (no section there yet), so there's no way to browse into a "Today" collision in this case.
- **Opened from a past date, no section today or in the future:** "Today" alone, always offered, from every occurrence in the strip (today and the future can't collide with it since neither is present).

New tests: `controller.test.ts` (+2, `carryHistorySelectionForward` same-file cases — an open tab and a disk-only file, confirming one combined write); `search-and-history.spec.ts` (+3: the bug's own repro end to end, the "section exists today" matrix case exercising all three browsing positions, and the "only a future section exists" case). One pre-existing test's assertion text updated for the reworded fallback message.

Verification: `svelte-check` 257/0; Vitest full suite 584/584; `search-and-history.spec.ts` 33/33, plus `mobile-ergonomics.spec.ts`, `modal-system.spec.ts`, `icon-system.spec.ts`, `action-drawer.spec.ts`, `tabs-lifecycle.spec.ts` all green (the last five as a sanity check on the shared modal-header CSS change) — all single-worker.

## 215. Section History: fixes the "opens on the oldest date" bug, adds scroll arrows, a connected active tab, and pinned today/source tabs

More chat feedback from actually using the redesigned strip.

**Bug: opening the drawer focused the oldest date, not the one it was opened from.** The focus-on-open logic ran once in `onMount`, searching `occurrences` for a match — but #62 means `historyOccurrences` always starts `[]` and fills in only once the disk read resolves, genuinely *after* `onMount` runs in every real-world case (that's the whole point of the spinner: the modal never waits for the read before opening). The search found nothing, left `selectedIndex` at its default of 0, and nothing ever re-ran it — which, now that §213 sorts the strip oldest-first, put the focus on the *oldest* occurrence instead. Fixed by moving the logic into a reactive statement that fires the first time `occurrences` actually has anything in it, rather than a one-shot check in `onMount` that can run before the data exists. A regression test forces the race deterministically with the same `delayCommands` pattern the #62 tests already use — the existing (fast, undelayed) "opens focused" test had been passing throughout, since a fast mock read can resolve quickly enough to coincidentally mask the bug.

**"I miss the left and right buttons that the main tab bar has."** Added `.history-occ-scroll-btn`s either side of the strip, shown once it genuinely overflows, scrolling by a fixed step and wrapping at either end — the exact same behavior `#tab-bar`'s own `.tab-scroll-btn`s already have (`TopBar.svelte`'s `scrollTabBar`), including that they only scroll the view and never change which occurrence is selected. A `ResizeObserver` on the strip (window-width-driven overflow changes) plus a reactive re-check on `occurrences` itself (content-driven changes, which don't necessarily resize the strip's own box) keep the arrows' visibility correct either way.

**"Visually connect the tab buttons to the content."** The active occurrence tab's background now matches `.history-detail`'s own (`--surface-canvas`, not the strip's `--surface-raised` hover tone) and its bottom corners square off (`border-radius: 5px 5px 0 0`) — it reads as the same surface the note body belongs to, a folder-tab look, rather than just another highlighted button sitting in a separate row above an unrelated panel.

**"Keep today's tab button and destination section tab button always in view... even when moving to earlier or later dates."** `.pinned-today` and `.pinned-source` (the occurrence matching `$currentDateISO` and the one History was opened from, respectively) get `position: sticky` with *both* `left: 0` and `right: 0` — the standard "frozen first/last column" recipe: whichever edge a pinned tab's natural position would otherwise scroll past is the one it clamps to, so it stays visible browsing in either direction, not just one. (An occurrence that happens to be both — opened from today — just gets both classes, which is a real, well-defined combination for `position: sticky`.) A dedicated test browses 20 places away from both pinned dates and asserts their bounding boxes are still within the strip's own visible viewport, not just that the CSS classes are present.

Verification: `svelte-check` 257/0; `search-and-history.spec.ts` 37/37 (+4: the focus-on-open regression test, the scroll-arrow test, the visual-connection test, and the pinned-tabs-stay-visible test), plus `mobile-ergonomics.spec.ts`, `tab-scroll-arrows.spec.ts`, `modal-a11y.spec.ts` green as a sanity check — all single-worker. Also manually verified live against the dev server (a real overflowing 12-date strip): the scroll arrows appear and wrap, and the opened-from tab's sticky positioning holds up under an actual scroll. **The active tab's background-matching part of this section was corrected the same day — see §216.**

## 216. Section History: the "visual connection" background made the active tab unreadable on a past date

Marien, immediately after §215: "I don't like how the visual connection was done. It's very hard to see the active tab now if that is a past date as it is gray on gray, and today is bold and blue."

Root cause, found by actually reading the color tokens instead of assuming: §215 matched the active tab's background to `.history-detail`'s own `--surface-canvas` (`#1e1e1e`) to "connect" it to the note body below. But `--surface-canvas` is *darker* than the strip's own `--surface-chrome` (`#252526`) — the "highlighted" tab ended up **less** prominent than an ordinary unselected one, not more. Compounding it: the past-date dimming added in §213 (`opacity: 0.55`) was applied unconditionally to `.history-occ-date`, which is a tab's *only* text at all (unlike the main tab strip, where the equivalent past-dimming only ever touches a decorative icon, never the tab's own label) — so an active-and-past tab got a low-contrast background *and* dimmed text simultaneously, while an active-and-today tab was saved by its unconditional accent-blue color override (hence "today is bold and blue" reading fine).

Fixed in two parts: (1) the active background goes back to `--surface-raised` (`#37373b`, clearly lighter — the same token every other active/hovered control in the app already uses), abandoning the exact-color-match approach; the squared bottom corners (`border-radius: 5px 5px 0 0`) are kept as a geometric-only "belongs to the panel below" cue that doesn't depend on a color contrast that can break. (2) the past-date dimming is now scoped `.history-occ-tab.past:not(.active)` — dimmed for at-a-glance scanning of the *un*selected tabs (which is what "past" means everywhere else in the app), but the selected tab is always shown at full strength regardless of which date it is, matching how the main tab strip's own active state is never undermined by its past/future treatment.

New regression test browses to a past occurrence, asserts its background differs from the strip's own chrome tone (a real highlight exists) and its date text is at full opacity, then confirms the same background applies when today is selected instead — proving the active treatment doesn't vary by date-class. The prior test asserting the (wrong) exact background match was replaced with one that only checks the corner-radius connection cue, which is still correct.

Verification: `svelte-check` 257/0; `search-and-history.spec.ts` 38/38 (the background-match test rewritten, +1 new legibility regression test); full Vitest 584/584 unaffected (pure CSS + one test file). Manually re-verified live: an active-but-past tab (`2026-08-24` in a real 12-date strip) now shows the same clearly-lighter highlight as an active-today tab, both fully legible.

## 217. Section History: pinned today/source tabs reflow the strip instead of floating over it

Marien: "The pinned today/destination tab should not hover over the tab bar. Shrink the tab bar instead when the pinned tab goes out of view, and pin it where it was about to move out." A direct rejection of §215's `position: sticky` approach — sticky necessarily overlays whatever's scrolling underneath it (that's the entire mechanism), which is exactly what wasn't wanted here.

**Replaced with a genuine layout reflow.** Today's occurrence and the opened-from occurrence are now tracked as a `PinState` (`"visible" | "off-left" | "off-right"`), computed by comparing each one's real `getBoundingClientRect()` against the scrollable strip's own — not inferred from scroll position math, so it stays correct regardless of how the tab got out of view (arrow click, drag-scroll, keyboard). When either goes off-view, a small duplicate button (a "pinned slot," sharing the same `.history-occ-tab` styling) renders as an ordinary flex *sibling* of `.history-occ-strip`, on whichever side the real tab scrolled past — a true DOM reflow that narrows the strip's own available width to make room for itself, rather than a `z-index`ed layer floating above it. The two pins share one slot (`pinned-today pinned-source`) when today's occurrence and the opened-from occurrence are the same date, rather than doubling up. Clicking a pinned slot jumps straight to (and scrolls to reveal) the real tab it's standing in for, at which point the slot itself disappears — there's never a pinned slot for a tab that's already visible.

**A real Svelte timing race, found while building this.** The first draft occasionally computed pin state against a strip that hadn't rendered its tabs yet at all: `historyOccurrences` (the store driving the `{#each}`) populates *before* `historyLoading` flips to false (existing #62 behavior), so the reactive block that fires "focus the opened-from occurrence" can run while the template is still on its loading-spinner branch — zero `[data-occ-index]` elements in the DOM. A fixed one- or two-`tick()` wait (tried first) wasn't reliably enough ticks; replaced with `waitForStripRendered()`, which polls (up to 5 ticks) until the strip's actual rendered tab count matches `occurrences.length` before measuring anything.

**A second, genuinely app-level bug found chasing down a test failure, not just a test artifact.** `scrollOccStrip`'s wrap-to-the-far-end click computes its scroll target (`scrollWidth - clientWidth`) once, before scrolling. But if that scroll itself scrolls the opened-from/today tab out of view, a pinned slot pops in partway through the animation and narrows the strip — growing the *true* end target out from under the in-flight scroll, so it lands short of it (browsers don't retroactively adjust `scrollLeft` when `clientWidth` shrinks). Fixed with `scrollStripToEndCorrecting()`: after the initial smooth scroll, a `scrollend` listener (with a timeout fallback, in case the initial scroll turns out to already be a no-op) re-checks the target and issues a corrective instant scroll if it moved. A smaller, related fix: `atLeftEdge`/`atRightEdge`'s wrap-detection gained a 40px `EDGE_TOLERANCE`, since landing a few pixels short of a since-grown end (from the same mechanism) was enough to make the *next* click's own "am I at the edge" check fail and scroll normally instead of wrapping.

**Test found the design's own edge case, not just a bug in the fix.** `mobile-ergonomics.spec.ts`'s narrow-viewport History test asserted exactly 2 tabs inside `.history-occ-strip` — an assumption this redesign breaks on purpose (a narrow strip can legitimately pin one of two tabs outside it). Reworded to count `.history-occ-tab` across the whole modal instead, which is what the test actually cared about (both occurrences are represented somewhere, the layout doesn't overflow) rather than which specific element renders which date.

Verification: `svelte-check` 257/0; `search-and-history.spec.ts` 38/38 (the pinned-tab test rewritten around the new mechanism, using 30 seeded dates rather than 15 so scrolling to the oldest reliably pushes the opened-from tab out of view on its own); `mobile-ergonomics.spec.ts` / `tab-scroll-arrows.spec.ts` / `modal-a11y.spec.ts` green as a sanity sweep; full Vitest 584/584; `cargo test` 149/149 (unaffected — pure frontend). Full Playwright suite run as a final check before committing.

## 218. Section History: pinned-slot vertical misalignment, and take-over caret jumping to the top of a different target tab

Two more pieces of chat feedback, found immediately after §217 shipped.

**"The new pinned slots don't align with the rest of the tab bar. They are more to the top instead of on the same level."** Root cause: `.history-occ-strip-row` uses `align-items: stretch`, but `.history-occ-tab` sets an explicit `height: 26px` — per the flexbox spec, `stretch` only applies to a child whose cross-size is otherwise `auto`; a child with a definite height falls back to `flex-start` (the row's top edge) instead. The in-strip tabs never showed this because they're not direct children of the row at all — they're children of `.history-occ-strip`, which itself *has* no explicit height and so genuinely stretches to the row's full height, and then centers its own children via its own `align-items: center`. A `.pinned-slot`, being a sibling of that strip rather than a child of it, never goes through that second centering step, landing flush at the top instead. Fixed with one `align-self: center` on `.history-occ-tab.pinned-slot`. Verified live (not just by eyeballing a screenshot): seeded 30 dates via the mock's `write_note`, scrolled the strip so the opened-from date pinned out, and compared `getBoundingClientRect()` between a normal in-strip tab and the pinned slot — `top`/`height` now identical (were previously off by several pixels).

**"When taking over content from the section history to the active main tab, the caret moves to the top of the content area. Instead, the caret should go to where the content was inserted."** `commitCopyForward` (`copyForward.ts`) already restores the caret after writing the *source* tab (§75, when the source is the active tab — deferring a line doesn't add/remove lines, so the old line index is still valid) but had no equivalent for the *target* tab: `writeTabContent`'s full-document replace collapses the live editor's selection to line 0 by default, and nothing put it anywhere else when the target — not the source — happened to be the tab you were looking at (the ordinary case for take-over's "here" destination, which targets the very note History was opened from). Fixed by having `insertIntoSection` report back exactly which line its own insertion landed on (`insertedAtLine`, alongside the new content it already returned), and calling `editorApi.jumpToLine(insertedAtLine)` after writing the target whenever that target is the active tab — mirroring the existing source-side pattern rather than inventing a second one. New regression test (`search-and-history.spec.ts`) reads the status bar's `Ln N` position after a take-over and confirms it lands on the actual inserted line — confirmed to fail (`Received: 1`) against the pre-fix code via a throwaway `git stash` of just this file.

Verification: `svelte-check` 257/0; `search-and-history.spec.ts` 38/38; `controller.test.ts` 179/179 (unaffected — `carryHistorySelectionForward`'s existing coverage doesn't exercise a live editor, only content); full Vitest 584/584. Full Playwright suite run as a final check before committing.

## 219. Multilanguage support (Phase 1 of 3): the i18n foundation, wired end to end through the new Language setting

Design: `docs/design/i18n-roadmap.md`. Marien: support English, Dutch, and German, following the system language by default with a Settings override, and without touching keyboard shortcuts. **This section is the foundation only** — the mechanism now works end to end and is fully tested, but only one real UI surface (Settings' own Appearance tab) has actually been translated. The remaining ~25 Svelte components, ~55 toast messages, shortcut labels, and command palette entries move over incrementally in follow-up sections, not in one sweep.

**New `LanguageMode` (`storage.rs`) mirrors `ThemeMode` exactly** — `En | Nl | De | System` (default), a new `AppConfig.language_mode` field (`#[serde(default)]`, so an existing config loads unchanged), and a `set_language_mode` command. `ts-rs` picks up the new enum automatically, same as every other config enum. The usual 4-way frontend mirror — `tauriApi.ts`/`tauriCommands.ts`/`testing/mockBackend.ts`/`webapp/webBackend.ts` — got the same treatment `theme_mode` already has, plus a `languageMode` store (`stores.ts`) and `setLanguageMode`/`applyLocaleToDom` in `boot.ts` (sets `<html lang>` — a real accessibility fix, not just cosmetic).

**Translation mechanism is hand-rolled, not a library** (per the design doc's reasoning: a few hundred short strings across 3 simple-plural languages doesn't need ICU/lazy-loading machinery, and this project already leans on `svelte-check` rather than a linter for correctness). New `src/lib/i18n/`: `schema.ts` defines `TranslationParams` (one entry per key, with its parameter shape) and a `Dictionary` type derived from it; `locales/{en,nl,de}.ts` each `satisfies Dictionary` against the exact same key set — a key missing or mistyped in any one locale is a `svelte-check` error, not a silent runtime gap. `index.ts` detects the system locale once at boot from `navigator.language` (works identically on desktop's webview and the web app's real browser, no new Tauri plugin), and exposes `locale` (`languageMode === "system"` ? the detected locale : the explicit override) and `t` — both derived stores, so `$t("some.key")` in a template is reactive to a language change exactly the way `$themeMode`/`$colorMode` already are elsewhere.

**Settings' Appearance tab** gained a "Language" control directly under Theme (`System`/`English`/`Nederlands`/`Deutsch`, each language named in itself) and had its existing "Appearance"/"Theme" labels and hints converted to `$t(...)` calls, proving the whole mechanism against real, previously-hardcoded text rather than only new strings.

**A real test-determinism gap found along the way, not just for this feature.** `playwright.config.ts` never pinned a browser `locale`, so Chromium's reported `navigator.language` — and now, which language the UI renders in — followed the *host OS's* own locale. Confirmed live: this machine's own browser reports Dutch by default, which would have made the exact same spec render English in CI and Dutch locally. Fixed by pinning `locale: "en-US"` in the shared `use` config; `i18n-locales.spec.ts` explicitly overrides it per-test to exercise the other two languages. Also fixed two now-ambiguous `getByRole("radio", { name: "System" })` queries in `settings.spec.ts` (Theme and Language both have a "System" option now) by scoping to the Theme radiogroup specifically.

New `tests/e2e/i18n-locales.spec.ts`: an explicit override renders the right language and sets `<html lang>` regardless of the browser's own locale, and survives a reload; `system` (the default) actually follows the browser's reported language with no override set; and — the point of the whole exercise — `Ctrl+Shift+H` still opens Section History under a German-language seed, turning "shortcuts are unaffected by language" from a design claim into something CI checks. New `src/lib/i18n/dictionaries.test.ts` (Vitest) asserts all three locale files have identical key sets, as defense in depth beyond the compile-time check.

**Translation content**: Dutch (`nl.ts`) and German (`de.ts`) were drafted directly for this pass; per the design doc, `nl.ts` should get a read-through from Marien (a native speaker) and `de.ts` needs a separate native/fluent reviewer before this ships in a release — neither has happened yet.

Verification: `cargo test` 151/151 (+2: `language_mode_round_trips_through_json_for_every_variant`, `load_config_defaults_language_mode_when_omitted`); `svelte-check` 262/0; Vitest 586/586 (+2); full Playwright suite 400/400 (+3 new, +0 regressed — the two `settings.spec.ts` fixes keep their original assertions, just disambiguated). `npm run build` confirmed clean. Verified live in the Browser pane: Settings opened already in Dutch (this machine's own browser locale) with no override set, switching to German applied instantly and updated `<html lang>`, and the choice survived a reload.

## 220. Multilanguage support (Phase 1, batch 2 of N): the Shortcuts & Symbols drawer

First batch of the mechanical sweep §219 deferred — Marien: "Continue with the mechanical sweep, a few components at a time." This one: `shortcuts.ts`'s registry labels and the whole Shortcuts & Symbols drawer (`ShortcutsModal.svelte`), plus a shared `common.close` key.

**A real scoping question, answered by reading every call site rather than assuming.** `shortcuts.ts`'s own header comment claims its `label` field is read by "every place a shortcut is displayed... the command palette's hints, TopBar/StatusBar tooltips, the About drawer" — grepping every consumer showed this is stale: `commandPalette.ts`, `TopBar.svelte`, `StatusBar.svelte`, and `AboutModal.svelte` only ever call `formatShortcut`/`formatCombo` (the *key combo* text) and keep their own separately hand-written label strings for display. `ShortcutsModal.svelte` is the *only* real consumer of `.label`. That narrowed this batch to exactly two files instead of five, and means the command palette's own hardcoded labels are a separate, later batch.

**`ShortcutDef.label: string` removed from the registry entirely** — labels now live in the i18n dictionaries, referenced by a new explicit `SHORTCUT_LABEL_KEYS` map (`shortcuts.ts`) from each shortcut id to its `TranslationKey`, `satisfies Record<(typeof SHORTCUTS)[number]["id"] | "clickGlyph" | "escape", TranslationKey>` — a renamed or removed id is a `svelte-check` error, not a silently-blank drawer row. The two `DRAWER_ROWS` entries that aren't real registry ids (the "Click a glyph" mouse action, "Escape") now carry a pseudo-id (`"clickGlyph"`/`"escape"`) in place of their old literal description string, resolved through the same map — their `keys` column (`"Click a glyph"`, `"Escape"`) stays literal, on the same reasoning as a formatted key combo (`Ctrl`, `Shift`) never being translated: it's naming an input, not describing it in prose. `drawerRows.test.ts` rewritten to resolve label text through `SHORTCUT_LABEL_KEYS` + the English dictionary instead of a removed `.label` field.

**A real, honest scope boundary, not silently glossed over.** Several of the drawer's glyph-legend explanations ("Delegated", the section-headers hint) weave `<kbd>` examples into the *middle* of the sentence — translating them cleanly needs either `{@html}` (a pattern this codebase has never used, and a real security-posture decision worth making deliberately) or restructuring the schema to support multi-segment rich text. Both are punted to a dedicated later pass; these two strings stay hardcoded English for now, each with an inline comment saying so. Every other glyph explanation in the same modal — including "Numbered list", which only has its `<kbd>` examples at the very *end* of the sentence — translated cleanly by ending the dictionary string at a clean break ("Sub-items:") and appending the literal `<kbd>` elements in the template afterward, no `{@html}` needed. The "Consequence-action" row's live `{formatShortcut(...)}` interpolation became a genuine parameterized key (`shortcuts.modal.consequenceAction`, `{ shortcutHint: string }`) — the first real use of the schema's parameter-shape mechanism beyond the placeholder `history.linesSelected`/`updates.updatedToVersion` examples in the design doc.

New keys: 30 shortcut labels (29 registry ids + `escape`; `clickGlyph` is the 30th), plus the drawer's own chrome (title, mobile tab labels, 3 group headers) and 11 glyph-legend strings — 48 new keys total across `en.ts`/`nl.ts`/`de.ts`. `SettingsModal.svelte`'s own Close button also switched to the new `common.close` key while touching this area, for consistency with the drawer's identical button.

Verified live in the Browser pane, not just by test count: switched to German via Settings, opened the Shortcuts & Symbols drawer, and confirmed full German rendering — including the live-computed "Ctrl+1-4" hint substituting correctly into the consequence-action sentence — with exactly the two deferred rows correctly still in English.

Verification: `svelte-check` 263/0; Vitest 587/587 (+1: `dictionaries.test.ts` needed a params-aware rewrite once a real parameterized key existed); full Playwright suite 400/400 (unaffected — no spec asserts on drawer text beyond structural checks already covered).

## 221. Multilanguage support (Phase 1, batch 3 of N): the Action Drawer

Third batch of the ongoing mechanical sweep (§219–§220) — Marien: "Continue with the Action Drawer next." `ActionDrawerModal.svelte`'s full text surface: the modal's own accessible name, the filter placeholder, the open/listed counter, all three scope options (Open Tabs/Other Notes/All Files) and their tooltips, the "Only Open" toggle and its tooltip, both empty states, each row's line-number tag, and the footer's keyboard hint labels (`<kbd>` combos themselves stay literal, same reasoning as §220 — only the words after them translate).

**Two new shared `common.*` keys, found by checking for reuse before assuming this modal's strings were unique to it.** `aria-label="Close dialog"` turned out to be identical across **14** modals (grepped, not guessed) — added `common.closeDialog` and used it here plus retrofitted the two modals already touched in earlier batches (`ShortcutsModal.svelte`, `SettingsModal.svelte`); the other 11 pick it up naturally as their own batches come around, rather than being touched speculatively now. `aria-label="Loading"` on the spinner is shared with `HistoryModal.svelte` (2 modals) — added `common.loading` here, `HistoryModal` picks it up in its own later batch.

**Two new genuinely parameterized keys** (`actionDrawer.counter: { open, listed }`, `actionDrawer.empty.noMatch: { filter }`, `actionDrawer.item.lineTag: { line }` — three, not two) exercising the schema's interpolation mechanism beyond §220's single example. The "no actions match" message's curly smart-quotes (`"…"`) were switched to plain straight quotes in the translated version — different languages have different typographic quote conventions (Dutch/German both traditionally use low-high „…" guillemets, not the English curly pair), and reusing English's specific convention across all three would just be wrong in two of them; plain quotes are the correct pragmatic choice here, not a regression.

Verified live in German via the mock's `set_language_mode` console hook (`window.__CHRONO_MOCK__.core`) rather than clicking through Settings each time: the whole drawer's chrome rendered in German with a workspace that had zero actions, then typing a filter confirmed the parameterized "no match" message substitutes correctly (`Keine Aktionen stimmen mit "zzznomatch" überein.`).

Verification: `svelte-check` 263/0; Vitest 587/587 (+3 new sample params in `dictionaries.test.ts` for the three new parameterized keys); full Playwright suite 400/400 (unaffected — no spec depends on the smart-quote empty-state text).

## 222. Multilanguage support (Phase 1, batch 4 of N): Section History, and its own destination labels

Fourth batch — Marien: "Continue with Section History next, then do the others as well." The app's largest, most-iterated-on modal (`HistoryModal.svelte`, 697 lines, redesigned across §150/§209–214/§215–218): the title bar, the occurrence strip (scroll buttons, tab tooltips, loading/empty states), the take-over bar, both empty states, and the footer's keyboard hints.

**The first translation key that has to live outside a Svelte component.** `HistoryDestination.label` ("Add to today" / `Add to 2026-09-10` / `Add to "Scratchpad 1"` / `Add to next occurrence (2026-10-03)`) is computed in `history.ts`'s `computeHistoryDestinations()` — a plain async function, not a component, called once when the drawer opens rather than re-evaluated reactively. Used `get(t)(...)` (the same store, read imperatively) instead of `$t(...)`, matching how the rest of this store already behaves: like `historyOccurrences` itself, the label is computed at refresh time and doesn't hot-update if the language changes while the drawer happens to be open — an accepted, pre-existing tradeoff (nothing else in this store is reactive to external state changes mid-session either), not a new gap introduced by translating it.

**A real type error caught by `svelte-check`, not just a lint nag.** Passing `openedFromDate` (typed `string | null`) into a translation key requiring `{ date: string }` failed to compile — the code's own logic guarantees `openedFromDate` is non-null in that branch (it's only ever `null` when `isScratchpad` is true, and that branch is only reached when it's false), but TypeScript can't see the correlation between two different variables. Fixed with a documented non-null assertion, the same trust the original hand-written template literal (`` `Add to ${openedFromDate}` ``) was silently placing in that invariant all along — the difference is `svelte-check` now actually requires writing that trust down instead of a template literal quietly accepting `string | null` with no complaint.

**A real Playwright determinism bug found and fixed, distinct from §219's browser-locale one.** `tests/e2e/helpers.ts`'s `MODAL_LABELS`/`modalCard()` locate a modal by its *English* aria-label — safe for the other 399 specs (all under the default `languageMode: "system"` + the pinned `en-US` locale from §219), but this batch translated `HistoryModal`'s dialog `aria-label` (and, while touching `SettingsModal.svelte` again, its own — missed in §219, `aria-label="Settings"`/the visible title span were never converted, just the Appearance tab's content beneath them). That broke this file's own German-seeded "keyboard shortcuts are unaffected" test, which had used those same English-label helpers. Fixed by locating both modals via their stable CSS class (`.history-modal-card`/`.settings-modal-card`) instead, and turned the fix into a positive assertion — the test now explicitly checks the *translated* aria-label (`"Abschnittsverlauf"`/`"Einstellungen"`) rather than just avoiding the English one.

New keys: ~21 in `HistoryModal.svelte`, 4 destination labels in `history.ts`, plus the missed `settings.modal.title` (retroactively fixing §219's gap). `history.modal.dateCount` is a second real pluralization case (after §220's "Numbered list" wasn't one and §221's `actionDrawer.counter` wasn't either) — `{count} dates`/`1 date`, same simple-plural pattern nl/de both handle with a ternary.

Verified live: seeded German, typed a real section into the editor, and opened the drawer via `Ctrl+Shift+H` — full German rendering including the singular form (`1 Datum`, not `1 Daten`) and the "nowhere else to carry this to" take-over message.

Verification: `svelte-check` 263/0; Vitest 587/587 (+6 sample params for the new parameterized keys); full Playwright suite 400/400 (1 failure caught and fixed mid-batch — see above — before landing at 400/400 clean).

## 223. Multilanguage support (Phase 1, batch 5 of N): the Command Palette

Fifth batch — Marien: "Continue with Section History next, then do the others as well," continuing straight on. The Command Palette (`commandPalette.ts` + `CommandPaletteModal.svelte`, Ctrl/Cmd+K) is the app's largest static label set — ~28 command entries plus the modal's own chrome, legend chips, and both dynamic result modes (`@` date jump, `!`/`#` open-action search).

**A real logic-vs-display coupling caught before it became a bug.** `CommandPaletteModal.svelte` compares `PaletteItem.group` against literal English strings (`"Commands"`/`"Current line"`/`"Help"`) to decide which results get `.is-shortcut` styling — translating `group` itself, not just its rendered header, would have silently broken that styling under any non-English language. Fixed by keeping `group` an untranslated internal identifier (same role as `shortcuts.ts`'s own ids) and adding `COMMAND_PALETTE_GROUP_KEYS`, a display-only lookup from each group string to its translation key — mirroring `SHORTCUT_LABEL_KEYS`'s exact pattern from §220. `commandPalette.test.ts`'s existing assertions on `item.group` (`"Commands"`, `"Open actions"`, `"Current line"`) needed no changes at all, confirming the internal/display split held.

**Real reuse, not duplicate strings** — seven labels here are byte-identical to ones already translated in earlier batches, so they reuse those keys directly instead of adding new ones with the same text: `cmd-actions` → `actionDrawer.modal.ariaLabel`, `cmd-history` → `history.modal.ariaLabel`, `cmd-search` → `shortcuts.crossTabSearch.label`, `cmd-sync-calendar` → `shortcuts.syncCalendar.label`, `cmd-line-section` → `shortcuts.convertToSection.label`, `cmd-settings` (and the "Settings" group header) → `settings.modal.title`, `cmd-about` → `shortcuts.openAbout.label`. A real, if minor, naming collision surfaced while wiring `openTabItems()`: its `.map((t) => ...)` callback parameter was named `t`, the same name the new `t` translation store import now uses — renamed to `tab`.

**Second and third translation keys computed outside a Svelte component** (after §222's `history.ts` precedent): `commandItems()`/`openTabItems()`/`buildPaletteResults()` are all plain functions rebuilt fresh on every palette open/keystroke, not derived stores, so each reads `get(t)` once at the top the same way `history.ts` does — a real parameterized case here too (`commandPalette.jumpToDate: {date}`, the `@today`-style date-jump result).

Verified live in German: the whole palette's default view (all group headers, every command label, every hint), plus the `@` date-jump mode producing `"Zu 2026-09-26 springen"` with the date substituted correctly mid-sentence.

Verification: `svelte-check` 263/0; Vitest 587/587 (unaffected — `commandPalette.test.ts`'s group/label assertions target either the untranslated internal `group` id or dynamic user-content labels, neither of which changed); full Playwright suite 400/400, clean on the first run this time (no locale-coupling surprises left after §222's fixes).

## 224. Multilanguage support (Phase 1, batch 6 of N): the top bar and status bar

Sixth batch, continuing the sweep — Marien: "keep going until you are done." `TopBar.svelte` and `StatusBar.svelte` together, since both are top-level chrome dense with tooltips rather than any single dialog's content.

**The one real hazard in this batch, called out in its own new schema comment**: several `TopBar.svelte` icon labels ("Actions", "Section history", "Search", …) render in up to three places simultaneously — the live button, and two off-screen `.topbar-measure` clones the §161 collapse-prediction logic (`predictLabelsWouldFit`/`predictUncollapseWouldFit`) measures with `getBoundingClientRect()` to decide whether labels or the "More" button fit at the current window width. Those clones exist specifically so the app can know a translated label's *actual current-language width* before committing to showing it — every occurrence had to use the exact same `$t(...)` call, not just the visible one, or the measurement would silently drift from what's really on screen once the string length differs from English (German and Dutch labels here are routinely longer, e.g. "Abschnittsverlauf" vs. "Section history"). Verified by running the full `topbar-collapse.spec.ts` suite (which exercises this exact prediction logic) unmodified — all pass, confirming the reused-key discipline held.

**More real reuse**: `topBar.label.actions`/`.history`/`.settings` all reuse `actionDrawer.modal.ariaLabel`/`history.modal.ariaLabel`/`settings.modal.title` from earlier batches rather than duplicating identical English text (each already has its own Dutch/German translation, so this is free correctness, not just less typing) — the status bar's plain "About ChronoNote" title reuses `shortcuts.openAbout.label` the same way.

**One thing deliberately left untranslated**: the literal word "OneDrive" in the cloud-sync button's fallback label — a Microsoft product name, not prose, same treatment as "ChronoNote" itself gets everywhere else in the app.

New keys: ~26 in `TopBar.svelte` (tab-list/scroll/tab-status/window-control strings, five buttons' tooltip+label pairs each duplicated across the measurement clones as described above), ~25 in `StatusBar.svelte` (OneDrive status text across its five sync states, sync-conflict count, browser-storage badge, the position/selection/word-count/action-count readouts, and the update/about/shortcuts tooltips).

Verified live in German: the full top bar (`Datum · Aktionen · Abschnittsverlauf · Suche · Einstellungen`) and status bar (`Z. 1, Sp. 1 · 6 Wörter · Offen 1 · Erledigt 0 · Weitergeleitet`) both render correctly at the same window width the English version does — the collapse/label-fit logic held up correctly under the longer German strings.

Verification: `svelte-check` 263/0; Vitest 587/587 (unaffected); full Playwright suite 400/400 including the full `topbar-collapse.spec.ts` (§56/§57/§61's width-prediction tests) clean on the first run.

## 225. Multilanguage support (Phase 1, batch 7 of N): About, and a real reactivity gap fixed

Seventh batch — the About drawer (`AboutModal.svelte`, Ctrl/Cmd+Shift+,), its whole update-check state machine (§update-check).

**A real reactivity gap, caught and fixed rather than shipped.** `agoLabel(ms)` was a plain helper function (`"Checked 3 minutes ago"`) called from the template; rewriting its body to call `$t(...)` would compile fine and work on first render, but Svelte only tracks `$store` references written *directly* in a template expression or a `$:` block as that block's reactive dependencies — a `$t` reference buried inside a separately-defined plain function's body isn't visible to that analysis. Left as a plain function, the "checked N ago" text would go stale if the language changed while About happened to already be open, silently, since nothing would tell Svelte to re-run it. Fixed by turning it into a `$:` block with the same IIFE body (`agoText`, replacing `agoLabel(...)`) — Svelte's dependency tracking then sees the `$t` reference directly inside the block it's already analyzing, alongside `$updateLastChecked`. `chip`'s own status label already lived in an equivalent `$:` block, so it only needed `$t(...)` calls inserted into its existing body, no restructuring.

**A near-duplicate string caught before it multiplied**: "What's changed" (the available-update link) and "What's new" (the status bar's first-launch-after-update link, §225's own `statusBar.whatsNew` from batch 6) are different English phrases with different meanings, not the same string reused — added `about.whatsChanged` as its own key rather than forcing a reuse that would've been semantically wrong (a real mistake caught mid-edit and corrected before it landed, not a hypothetical).

**One line deliberately left partly untranslated, matching the design doc's own documented scope**: the install/check-error messages (`Couldn't install the update. ${message}` / `Couldn't check for updates. ${message}`) have their leading sentence translated but `${message}` itself stays whatever Rust's `Result<T, String>` error text says — English, per Phase 2's still-not-started scope (`docs/design/i18n-roadmap.md`'s Non-goals section, restated in §219).

New keys: ~36, covering the version card's ten status-chip labels, every one of the eight update-flow branches' hint/button text, the "checked ago" pluralized function, and the Links/Learn More sections (the latter's two sentences both start right after their own `<kbd>` combo, the same clean-split pattern §220's "Numbered list" established — no rich-text schema needed here either).

Verified live in German: the whole modal — version card, "Aktuell" chip, "Gerade eben geprüft." ago-text, Links, and the Learn More sentences with both shortcut combos substituted correctly.

Verification: `svelte-check` 263/0; Vitest 587/587 (+2 sample params); full Playwright suite 400/400, including all of `update-check.spec.ts`'s state-machine assertions (chip text, button labels, error states) unaffected under the pinned English locale.

## 226. Multilanguage support (Phase 1, batch 8 of N): the safety warning and cross-tab search

Eighth batch — the tab-close safety modal (`SafetyModal.svelte`, unresolved actions / unsaved scratchpad) and cross-tab search (`SearchModal.svelte`, Ctrl/Cmd+Shift+F).

**A fifth non-component translation site** (after §222/§223 x2/§225's precedents): `tabs.ts`'s `requestTabClose` composes the safety message from up to two independent reason fragments (a due-open-actions count, an unsaved-scratchpad warning) joined with "and" — translated as `safetyModal.reason.dueOpen`/`.scratchpad` fragments plus a `safetyModal.reasonJoiner` (`" and "`/`" en "`/`" und "`), assembled into the full sentence via `safetyModal.message`. Existing `controller.test.ts` assertions (`toMatch(/unresolved open action/)`, `toMatch(/scratchpad/)`) needed no changes — they're substring checks against the still-English text under the suite's pinned locale, and the composed result is byte-identical to before.

**More reuse across batches**: search's "Open Tabs"/"All Files" scope labels reuse `actionDrawer.scope.openTabs.label`/`.allFiles.label` verbatim (batch 3), and its result rows' `Ln N` tag reuses `actionDrawer.item.lineTag` (also batch 3) — three more keys avoided rather than duplicated.

**One small, real fix riding along**: the match counter's `{count} match(es)` was never actually pluralized — a lazy fixed suffix regardless of `count`, present since the feature shipped. Translating it into a real `count === 1 ? "1 match" : "${count} matches"` ternary (the same shape every other pluralized key in this sweep uses) fixes it in English too, not just in Dutch/German.

**One deliberately out-of-scope area, confirmed by checking, not assumed**: `search.ts`'s filter-chip labels (`is:open`, `tag:${val}`, `has:@${val}`, `since:${val}`, `before:${val}`) are literal search-syntax keywords the user types directly into the query — the same category as the app's `#`/`v`/`=>` token vocabulary, not prose, and translating them would break the actual grammar the search parser matches against. Left untouched, on the same reasoning already applied to the editor's tokens in the design doc's Non-goals.

New keys: `common.cancel` (a new shared key — the app's first "Cancel" distinct from "Close"/"Close dialog"), 7 in `safetyModal.*`, 6 in `searchModal.*`.

Verification: `svelte-check` 263/0; Vitest 587/587 (+5 sample params); full Playwright suite 400/400 clean.

## 227. Multilanguage support (Phase 1, batch 9 of N): the date picker, and month/weekday names go through Intl

Ninth batch — the calendar popover (`DatePickerModal.svelte`, Ctrl/Cmd+O), plus the design doc's own planned architectural piece: replacing hand-translated month/weekday arrays with `Intl.DateTimeFormat`.

**`date.ts`'s `MONTH_NAMES` array (12 hardcoded English strings) and the component's own local `WEEKDAYS` array are both gone**, replaced by two new exported functions — `monthName(locale, month)` and `weekdayAbbrev(locale, weekdayIndex)` — that call `Intl.DateTimeFormat` directly. This was always the design doc's stated plan (less translation surface, grammatically correct forms for free), and this batch was the first natural place to actually do it, since the date picker was `MONTH_NAMES`'s only consumer. Verified against real `Intl` output, not assumed: Dutch month names come back correctly lowercase (`"september"`, not `"September"` — a real Dutch typographic convention, confirmed live and in a new `date.test.ts` case), and the Monday-first weekday indexing (`weekdayIndex 0` = Monday, matching `monthGrid`'s own convention) checks out against a fixed real Monday (2024-01-01) used purely as a reference point.

**No template rich-text problem this time** — the per-day `aria-label` builds a suffix (`", all tasks completed"` / `", open actions pending"` / etc.) entirely in a plain ternary chain in the script, not interleaved with any `<kbd>` markup, so it translated directly with no schema changes needed beyond one key per branch.

New keys: 12, covering the popover's own aria-labels/placeholder, the month-navigation buttons, the loading-older-notes state, all four day-heat aria-label suffixes, and the footer's "Today"/"Esc to close". `date.test.ts` gained a small dedicated suite for the two new `Intl`-backed functions.

Verified live in German: the calendar renders "September 2026" and the correct German weekday abbreviations (`Mo Di Mi Do Fr Sa So`) from `Intl` directly, plus "Heute"/"Esc zum Schließen".

Verification: `svelte-check` 263/0; Vitest 589/589 (+2, the new `monthName`/`weekdayAbbrev` tests); full Playwright suite 400/400 clean (`DatePickerModal.test.ts` had no dependency on the removed arrays to begin with).

## 228. Multilanguage support (Phase 1, batch 10 of N): four small modals — More Actions, Conflict, Unsaved Scratchpads, Dropped Notes

Tenth batch — four smaller, self-contained dialogs together: the top bar's overflow menu (`MoreActionsModal.svelte`), the on-disk-conflict warning (`ConflictModal.svelte`), the unsaved-scratchpads gate shared by app-close and folder-switch (`UnsavedScratchpadsModal.svelte`, §39/§93), and the drag-and-drop dropped-notes reviewer (`DroppedNotesModal.svelte`).

**Heavy reuse — this batch added only 2 new keys for More Actions**, everything else in that menu already had a translation from an earlier batch (`actionDrawer.modal.ariaLabel`, `history.modal.ariaLabel`, `shortcuts.crossTabSearch.label`, `shortcuts.syncCalendar.label`, `settings.modal.title`, both `topBar.calendarSync.title*` variants) — this menu is, after all, just the same top-bar actions relocated into an overflow popover once the window narrows, so its labels were always going to be the same strings.

**A clean split around dynamic data, not prose or markup**: `DroppedNotesModal`'s "You already have a note for **{filename}**, and the file..." sentence splits cleanly into a `.before`/`.after` pair around the `<strong>`-wrapped value, since what's embedded is a *filename* (never itself translated) rather than another phrase or a `<kbd>`/`<em>` UI reference — a cleaner case than §220's/this same file's own harder one below. Similarly, `droppedNotes.keepBothHint` takes the *already-translated* "Keep both" label as a parameter (`{ keepBothLabel }`) rather than hardcoding the phrase a second time inside the hint sentence, so the two can never drift out of sync in any language.

**A third rich-text deferral, same reasoning as §220's two**: `ConflictModal`'s explanatory paragraph weaves two `<em>`-wrapped button-name references (`<em>Keep disk</em>`, `<em>Keep mine</em>`) into the middle of one sentence — left English for now, flagged with the same kind of comment as the Shortcuts drawer's two deferred strings, pending the eventual `{@html}`-or-richer-schema decision.

New keys: 2 in `moreActions.*`, 5 in `conflictModal.*`, 6 in `unsavedScratchpads.*`, 12 in `droppedNotes.*`.

Verification: `svelte-check` 263/0; Vitest 589/589 (+2 sample params); full Playwright suite 400/400 clean.

## 229. Multilanguage support (Phase 1, batch 11 of N): move-to-OneDrive prompt and the in-document find bar

Eleventh batch — the web app's "Move Notes to OneDrive?" migration prompt (`MigrateNotesModal.svelte`) and the in-document find bar (`FindBar.svelte`, Ctrl/Cmd+F).

**The "split around dynamic data" pattern from §228 applied twice more, cleanly**: both of the migration prompt's explanatory sentences wrap a dynamic value (a pluralized note count, the target folder path) in `<strong>` — each splits into a `before`/`after` pair around its own value, exactly like `droppedNotes.hasNoteHint` did, with no rich-text ambiguity since what's embedded is data, not another phrase or a UI-element reference.

New keys: 12 in `migrateNotes.*` (aria-label/title, the count/folder sentence halves, the disclaimer, and both buttons' idle/busy label pairs), 10 in `findBar.*` (placeholder, aria-labels, the match-count readout, and the three nav buttons' aria-label/title pairs).

Verification: `svelte-check` 263/0; Vitest 589/589 (+2 sample params); full Playwright suite 400/400 clean.

## 230. Multilanguage support (Phase 1, batch 12 of N): calendar sync review and OneDrive sync conflicts

Twelfth batch — the calendar sync review (`CalendarSyncReviewModal.svelte`, #78) and the OneDrive sync-conflict resolver (`SyncConflictsModal.svelte`), a near-twin of §228's `DroppedNotesModal`.

**Reuse across a near-identical pair of dialogs**: `SyncConflictsModal` and `DroppedNotesModal` share the same "pick a version" shape (a side-by-side/mine/theirs view switcher, a "keep both" option with the same trailing-marker-line explanation), so this batch reused `droppedNotes.tab.sideBySide`, `droppedNotes.highlightedDiffer`, and `droppedNotes.keepBoth` directly rather than re-declaring three more identical English strings — the same parameterized-hint trick from §228 (`syncConflicts.keepBothHint` takes the already-translated "Keep both" label as a parameter) carried over too.

**"OneDrive" stays a literal brand name** in both the tab and the version badge, same treatment as the status bar (§224) — not given a translation key at all, since it's a product name, not prose.

New keys: 13 in `calendarSyncReview.*` (the review's four section headers, two composed reordered/removed-item sentences, the three Leave/Discard/Move choices, and the empty/confirm states), 8 in `syncConflicts.*` (the rest, after the three-way reuse above).

Verification: `svelte-check` 263/0; Vitest 589/589 (+3 sample params); full Playwright suite 400/400 clean.

## 231. Multilanguage support (Phase 1, batch 13 of N): the sync health popover, and relative times go through Intl too

Thirteenth batch — the cloud sync telemetry popover (`SyncHealthPopover.svelte`, clicking the status bar's cloud icon), plus a second `Intl`-based architecture win after §227's month/weekday names.

**`formatRelativeTime()`'s five hand-translated fragments (`"1 minute ago"`, `"${n} minutes ago"`, `"1 hour ago"`, …) are gone**, replaced by `Intl.RelativeTimeFormat(locale, { numeric: "always" })` — verified against real output for all three languages before wiring it in (`"3 minutes ago"` / `"3 minuten geleden"` / `"vor 3 Minuten"`, `"5 days ago"` / `"5 dagen geleden"` / `"vor 5 Tagen"`), confirming the correct word order and inflection come for free. Only two entries stay in the dictionary: `"Never"` and the "just now (HH:MM)" case, which embeds an absolute time no plain relative-time unit expresses on its own. `toLocaleTimeString` also switched from `[]` (the browser's own ambient locale) to the app's resolved locale explicitly, for the same reason `Intl.DateTimeFormat` calls elsewhere in this sweep pass a real locale rather than relying on an unrelated default.

**The exact same reactivity fix as §225's `agoLabel`, applied a second time**: `formatRelativeTime` is a plain function called from the template, so a `$t`/`$locale` reference *inside its own body* wouldn't be tracked as a dependency of the calling template expression. Fixed the same way — but this time by passing `$locale` and `$t` in as explicit arguments at the call site (`formatRelativeTime($syncHealth?.lastSyncSuccessMs, $locale, $t)`) rather than restructuring into a `$:` block, since the function is a pure formatter with no other reason to live inside reactive component state. Either fix works; which one reads better depends on whether the function has a natural reason to be a `$:`-computed value already (`AboutModal`'s `agoText` did) or is a pure utility that just needs its inputs made explicit (this one).

New keys: 20 in `syncHealth.*` — the popover's chrome, status labels, all six detail-row labels, the sign-in-expired hint, and the sync-now/open-settings buttons — plus several reused directly from `statusBar.oneDrive.*` (§224) for the states both surfaces already describe identically (syncing, sign-in-again, sync error).

Verification: `svelte-check` 263/0; Vitest 589/589 (+2 sample params); full Playwright suite 400/400 clean.

## 232. Multilanguage support (Phase 1, batch 14 of N): the OneDrive folder picker

Fourteenth batch — `OneDriveFolderPickerModal.svelte` (480 lines, mostly CSS), the web app's breadcrumb folder browser used when connecting or switching a OneDrive notes folder. The last of the large modals; everything remaining in the sweep is either small or lives outside a `.svelte` file entirely.

**A distinction worth being precise about, now that this file mixes both kinds of `$t` usage**: `$t` read inside a plain event-handler function (every `showToast(...)` call here, fired from a click/submit) is completely safe and needs none of §225/§231's reactivity fixes — those fixes matter only when a value is supposed to re-render reactively *while the component stays mounted with no user interaction*, which a toast triggered by a click is not. `$t` always reads the store's current value at the moment the surrounding code actually runs, so a click handler reading `$t` gets whatever language is active *right then*, correctly, with no extra plumbing.

**Toast prefixes that concatenate a Rust/API-originated error message** (`Failed to create folder: ${e.message}`, `Couldn't switch folders: ${e.message}`, etc. — 5 of them here) keep that trailing part in English, consistent with the Phase 2 deferral §219 already established; only each toast's static lead-in phrase translates.

**One remaining string deliberately left for a later pass, not this one**: the `"your OneDrive"` fallback name used when starting a folder switch flows into `directory.ts`'s `folderSwitchNote()` — a different file, generating scratchpad *content* text rather than this modal's own UI, so it's left for the dedicated toast/message sweep across `.ts` files rather than pulled in here.

New keys: 21 (the picker's chrome, breadcrumb root label, folder-list states, the inline new-folder form, the footer, and 8 toast messages/prefixes — 3 fully parameterized with correct pluralization, 5 with an untranslated error-message suffix as described above).

Verification: `svelte-check` 263/0; Vitest 589/589 (+3 sample params); full Playwright suite 400/400 clean.

## 233. Multilanguage support (Phase 1, batch 15 of N): the mobile accessory bar and tab drawer — every Svelte component now translated

Fifteenth batch — the touch-only mobile components, `MobileAccessoryBar.svelte` (the token/formatting toolbar above the on-screen keyboard) and `MobileTabDrawer.svelte` (the phone-width tab switcher). With these two done, **every `.svelte` file in the app has been swept** — confirmed by listing all 26 components and checking the three untouched ones (`AppIcon.svelte`, `EditorPane.svelte`, `Segmented.svelte`) contain no hardcoded user-facing strings at all (`AppIcon` is pure SVG markup, `Segmented` only ever renders labels its caller passes in, `EditorPane` is CodeMirror wiring with no chrome text of its own).

**Word-plus-literal-glyph split, the same pattern as the Shortcuts drawer's glyph legend (§220)**: each accessory-bar token button's `title` pairs a translated word with the literal token glyph itself (`{$t("mobileAccessory.openTask.titleWord")} ☐`, etc.) — the glyph stays untranslated token vocabulary, only the describing word moves.

New keys: 19 in `mobileAccessory.*` (six token buttons' aria-label/title-word pairs, indent/dedent, undo/redo), 7 in `mobileTabDrawer.*` (aria-label/title, close-tab-list, the in-memory-only tag, a parameterized "Close {tab}" aria-label, and the two drawer actions) — plus reuse of `datePicker.today` and `commandPalette.modal.ariaLabel` for text that's identical to strings translated in earlier batches.

Verification: `svelte-check` 263/0; Vitest 589/589 (+1 sample param); full Playwright suite 400/400 clean.

**Remaining for Phase 1**: roughly 55 toast-message call sites across `actions.ts`/`boot.ts`/`calendarSyncActions.ts`/`exportImport.ts`/`updates.ts`/`oneDriveSync.ts`/`syncConflicts.ts`/`directory.ts`/`menu.ts` and similar files — no more Svelte components to sweep, just the imperative-message layer underneath them.

## 234. Multilanguage support (Phase 1, batch 16 of N): the toast-message sweep begins, and a real crash-on-fallback bug fixed

Sixteenth batch — the start of the final piece of Phase 1: `showToast(...)` call sites in plain `.ts` modules, not Svelte components. Nine files done this batch: `actions.ts`, `boot.ts` (including its 9 near-identical "Failed to save X preference" toasts and the OneDrive sign-in flow), `calendarSyncActions.ts`, `copyForward.ts`, `directory.ts` (including `folderSwitchNote()` — real scratchpad *content* shown during a folder switch, not just a toast), `drift.ts`, `exportImport.ts`, `history.ts`, and `paste.ts`.

**A real crash found and fixed, not hypothetical.** Wiring `drift.ts`'s toasts to `get(t)(...)` immediately surfaced an unhandled exception in the Vitest suite: `DICTIONARIES[$locale]` was `undefined` because `$locale` itself was `undefined`. Root cause: `controller.test.ts` predates `languageMode` and mocks `api.getConfig()` with hand-built config objects that don't include it — loosely-typed `vi.fn()` mocks meant `svelte-check` never caught the gap. `boot.ts`'s `languageMode.set(cfg.languageMode)` then pushes `undefined` straight into the store, and `locale`'s derive function only special-cased the literal string `"system"`, not "anything that isn't a real locale." **Fixed at the source, not by patching test fixtures**: `locale` now falls back to `systemLocale` for *any* value that isn't a real supported locale, not just `"system"` exactly — the same defensive spirit as this codebase's `#[serde(default)]` fields (a config from before a field existed must behave exactly like that field's own default, not crash). This means any future test fixture, hand-edited config, or genuinely corrupt `languageMode` value degrades gracefully to the system language instead of taking down the whole translate function. Verified: the exact same Vitest run that threw the unhandled rejection before the fix ran clean after it, with all 589 tests still passing (this was never a *failing* test — Vitest doesn't fail a suite for an unhandled rejection outside an assertion — it was a real bug that would only have surfaced as a broken toast in production against an old or hand-edited config).

**Compositional messages, several new instances of the same pattern §226 established**: `exportImport.ts`'s import-summary toast (`"Imported 3 notes, skipped 1."`) builds its parts list from translated fragments and joins them with a literal comma, same as `safetyModal.message`'s reason-joining; `copyForward.ts`'s "Copied {dest}." message parameterizes `dest` itself (either a translated "here" or a translated "to {date}") rather than choosing between four hardcoded template variants. Three more genuinely correct singular/plural pairs went in along the way, including one (`exportImport.ts`'s "1 differs from what you have" vs "N differ from what you have") that's actually subject-verb agreement, not simple pluralization — preserved exactly as the original code already had it right.

**Toast prefixes concatenating a Rust/API-originated error message** (OneDrive sign-in failures, export/import failures) keep that trailing part in English, the same Phase 2 deferral as every other batch.

New keys: ~35 across `toast.actions.*`, `toast.boot.*` (14: OneDrive connect flow + 9 failed-to-save variants), `toast.calendarSync.*`, `toast.copyForward.*`, `toast.directory.*` + `directory.folderSwitchNote`, `toast.drift.*` (7), `toast.exportImport.*`, `toast.history.*`, and `toast.paste.*` (3 pluralized).

Verification: `svelte-check` 263/0; Vitest 589/589 (+13 sample params, clean of the unhandled-rejection noise after the `locale` fallback fix); full Playwright suite 400/400 clean, including `drag-drop-import.spec.ts`'s exact-text assertions on the now-composed-from-keys import-summary toast.

## 235. Multilanguage support (Phase 1, batch 17 of N): the toast-message sweep finishes — every Svelte component and every `.ts` call site is now translated

Seventeenth and final translation-sweep batch. Remaining files done: `tabs.ts` (3 toasts — reopen/promote-scratchpad flow), `syncConflicts.ts` (the resolve-conflict toast and its three-way kept-version ternary), `oneDriveSync.ts` (5 toasts — folder-not-chosen, sync finished/failed, sign-in failures), `updates.ts` + `StatusBar.svelte` together, and `persistence.ts`'s two identical "Failed to save note" autosave-failure toasts (found by a final exhaustive `grep` for every remaining `showToast(` call site across `src/lib/**/*.ts`, confirming nothing was missed and that `menu.ts` — named in earlier batches' "remaining" estimate — never actually had any translatable text to begin with).

**A fourth instance of the logic-vs-display coupling pattern (after `SHORTCUT_LABEL_KEYS`/§220, `COMMAND_PALETTE_GROUP_KEYS`/§223, and the toast-identity check is really the same shape as both)**: `updates.ts` used to export a literal string constant, `UPDATE_AVAILABLE_TOAST`, purely so `StatusBar.svelte` could compare it against the live toast text with `===` and decide whether to render that one toast as a click-to-About link. Translating the string itself would have broken the comparison the moment the active language wasn't English. Fixed by exporting a translation *key* instead (`UPDATE_AVAILABLE_TOAST_KEY`, resolving to a new `toast.updates.updateAvailable`) — `updates.ts` calls `get(t)(UPDATE_AVAILABLE_TOAST_KEY, undefined)` when actually showing the toast, and `StatusBar.svelte`'s comparison becomes `$toastMessage === $t(UPDATE_AVAILABLE_TOAST_KEY, undefined)`, read directly in the template so both sides always resolve against the same live locale — no separate reactivity fix needed (§225/§231's fix only matters for a value read *inside a plain function body*; this is a bare `$t(...)` call written straight into the template expression, so Svelte already tracks it as a dependency the normal way).

**One more genuinely correct three-way branch, not just prose**: `syncConflicts.ts`'s "kept X version of {name}" toast already had the right three outcomes (theirs/both/mine) — translated as three separate parameterized keys rather than one key with an embedded ternary, keeping each language free to reorder or reword any one branch independently.

**Toast prefixes concatenating a Rust/API-originated error message** (OneDrive resolve-conflict failures, sync failures, sign-in failures) keep that trailing part in English, the same Phase 2 deferral as every earlier batch.

New keys: 3 in `toast.tabs.*`, 4 in `toast.syncConflicts.*`, 4 in `toast.oneDriveSync.*`, 1 `toast.updates.updateAvailable` (replacing the old exported string constant), 1 `toast.persistence.failedToSaveNote`.

**Phase 1's translation sweep is now structurally complete**: every `.svelte` component (15 batches, §219–§233) and every `.ts` module's toast/message call site (this batch and §234) reads from the dictionary. What's left before any of this can ship: a native-speaker review pass on `nl.ts`/`de.ts` (Marien for Dutch, a separate German reviewer — neither done by me, per the design doc's own stated scope) and the still-not-started Phase 2 (translating the Rust/API error-message tails this sweep deliberately left in English throughout).

Verification: `svelte-check` 263/0; Vitest 589/589 (+7 sample params); full Playwright suite re-run twice after this batch's edits — the second, post-`persistence.ts`-fix run is the clean baseline; a first run showed 10 failures (`topbar-collapse.spec.ts` x2, `undo-history.spec.ts`, `update-check.spec.ts` x4, `visual.spec.ts` x2, `zen-mode.spec.ts`), all matching this project's documented flake-under-full-suite-parallel-load pattern — confirmed by re-running `update-check.spec.ts` alone (the file most directly touched by the `UPDATE_AVAILABLE_TOAST_KEY` change) in isolation: all 22 of its tests passed cleanly, so the four failures were load-induced timing, not a regression.

## 236. Multilanguage support, Phase 2 begins: a real error-code shape for the handful of Rust-authored error sentences, first slice

Phase 2 (`docs/design/i18n-roadmap.md`'s own explicitly-deferred item) starts: moving the small set of *fixed, Rust-authored* English error sentences off plain `String` and onto a real, translatable code, so the frontend can show them in the user's own language instead of always-English text — while leaving the overwhelming majority of fallible operations (`std::io::Error`, network/Graph-API errors) exactly as they were: raw, untranslated diagnostic text, since translating those would mean either inventing a generic bucket that throws away real diagnostic detail, or translating a remote server's own English response body. Neither is worth doing.

**New `src-tauri/src/error.rs`: `AppError`**, a `#[serde(tag = "code")]` enum with `impl From<String> for AppError` (→ `AppError::Other { detail }`) so every existing `.map_err(|e| e.to_string())`/`?`-chain converts automatically at whichever function boundary now returns `AppError` — none of those inner chains needed to change at all. ts-rs exported (`storage.rs`'s codegen list) as `{ code: "agendaInvalid" } | { code: "oneDriveSyncBusy" } | { code: "other", detail: string }`.

**This first slice covers exactly two real cases**, chosen because they're each one clean, dynamic-content-free sentence (no interpolated raw text forming the message's actual identity) reachable from a real user action:
- `AgendaInvalid` — `agenda.rs`'s single hardcoded sentence, now shared by `read_agenda_for_date`/`read_agenda_removed_for_date`/`read_agenda_after` (all three changed from `Result<_, String>` to `Result<_, AppError>`).
- `OneDriveSyncBusy` — the "try again in a moment" busy-guard, unified across *two* call sites that used to have two different hand-written strings: `OneDriveManager::sync_now`'s own guard ("Sync already in progress") and `resolve_conflict`'s ("A sync is running — try again in a moment"). One shared code, one shared translated message — the user-facing meaning was already identical.

**`OneDriveSyncResult.message` and `onedrive_resolve_conflict`'s Tauri-boundary error both changed from `Option<String>`/`Result<_, String>` to `AppError`-shaped.** `resolve_conflict`'s own body stays almost untouched — its busy-guard constructs the new variant directly, and everything else (`resolve_conflict_files`/`save_cache`, still plain `String`) converts via the blanket `From<String>` at one `.map_err(AppError::from)` on the way out. `OneDriveLoginResult.error` and `FolderSwitchResult.message` are **deliberately not touched yet** — see below.

**New `src/lib/apiError.ts`: `describeApiError(e)`** — the frontend's one place that turns an `AppError`-shaped value into display text: a known code (`agendaInvalid`/`oneDriveSyncBusy`) resolves through a new `error.*` translation key; `"other"` (and anything not shaped like an `AppError` at all — a plain `Error`, a bare string, every call site this slice didn't touch) falls back to exactly the pre-Phase-2 behavior. Wired into `calendarSyncActions.ts` and `copyForward.ts` (both read the agenda) and `syncConflicts.ts` (resolve-conflict failures) in place of their old `e instanceof Error ? e.message : String(e)` fallback chains.

**A real, previously-invisible bug found in the process**: on the *real* desktop app, a Tauri command's `Err(String)` rejects the JS promise with a plain string, never an `Error` instance — so `e instanceof Error` was always `false` there, meaning `calendarSyncActions.ts`'s detailed agenda-error text was **only ever shown when running against the mock** (whose hand-thrown `new Error(...)` *is* an `Error` instance) and silently replaced by a generic "Couldn't read the calendar." on every real install. `describeApiError` fixes this for good, not just for this slice's two codes — a rejection that isn't `AppError`-shaped still degrades to the exact same `instanceof Error` check as a last resort, so nothing regresses for the untouched call sites either. The now-redundant `toast.calendarSync.couldntReadCalendar` key (dead code once every consumer switched to `describeApiError`) was removed rather than left orphaned.

**The web app's own parallel TypeScript sync engine (`webOneDriveSync.ts`) and IndexedDB-backed agenda reader (`webBackend.ts`) mirror both codes too** — `OneDriveSyncResult` is a *shared* type (from `../types`, generated from the same Rust enum), so the web engine's own busy-guards and `webBackend.ts`'s six `.agenda.json`-invalid throw sites needed updating regardless, not by choice: leaving them as bare strings would have been a `svelte-check` type error, not a passable inconsistency. Both mocks (`mockBackend.ts` for Playwright/dev, and `webBackend.ts` itself, which is real web-app code, not a mock) now throw/return the identical `{ code: "..." }` shape a real Tauri backend does.

**Deliberately not covered by this slice (flagged, not silently dropped)**: the OneDrive sign-in flow's ~13 other fixed sentences across `onedrive::auth`/`onedrive::sync` (loopback bind failure, browser-open failure, OAuth timeout, missing refresh-token scope, etc. — all real, all fixed sentences, none touched yet) and `FolderSwitchResult`'s composed three-part "Couldn't sync {folder} before switching ({why}). Nothing was changed." sentence (which also has a `held` conflict-count that deserves real pluralization, and a parallel implementation in `webOneDriveSync.ts` to keep in sync) — both substantially larger in scope than this slice and queued as their own follow-up batch rather than rushed alongside everything else, especially given the folder-switch path's data-safety history. `error.rs`'s own module doc comment records this reasoning for whoever picks up the next batch.

New Rust tests (`error.rs`: 5, `sync.rs`: 2 — `resolve_conflict`'s busy-guard reports the code, an unrecognized resolution still falls back to `Other`); no new Vitest tests needed (`dictionaries.test.ts`'s existing completeness check already covers the two new undated `error.*` keys).

Verification: `cargo test` 158/158 (+7); `svelte-check` 263/0; Vitest 589/589; full Playwright suite 400/400 clean, including `sync-health-popover.spec.ts`'s "Expired OneDrive sign-in" test (verifies the sign-in-expired detection still works through the new wrapped shape) and `onedrive-first-connect.spec.ts`'s sync-finished assertion.

## 237. Multilanguage support, Phase 2 continues: the OneDrive sign-in flow's 13 fixed sentences

Marien: "Go." Picks up exactly where §236 left off — the OneDrive sign-in flow's own fixed sentences, flagged there as deliberately deferred rather than dropped.

**13 new `AppError` variants**, one per fixed sentence across `onedrive::auth::exchange_code` (token request/rejection/unparseable-response/missing-refresh-token-scope) and `onedrive::sync`'s three login functions (`login_interactive`: loopback bind failure, browser-open failure, callback-accept failure, auth timeout; `exchange_code_direct`: no auth code, no pending session; `finish_login`: keychain-save failure, auth-state-save failure, profile-fetch failure). `OneDriveLoginResult.error` changed from `Option<String>` to `Option<AppError>` to carry them. `auth.rs::exchange_code` itself changed from `Result<_, String>` to `Result<_, AppError>` — the one function in this flow whose signature had to change, since it's the one place that actually distinguishes "network request failed" from "Microsoft rejected it" from "response didn't parse," not just a passthrough. `refresh_access_token` (a *different* function, used only by a background sync's own token refresh, never by `OneDriveLoginResult`) deliberately keeps its plain `String` — its failures were always going to fold into `Other` regardless, since they surface via `OneDriveSyncResult.message`'s already-generic bucket, not a dedicated field.

**The web app's own parallel implementations needed the identical codes, not as a courtesy but because the type is shared**: `webOneDriveAuth.ts::exchangeCode` now throws `AppError` values directly (not wrapped in `Error`) for its own token-request/rejection/missing-scope failures, mirroring `auth.rs::exchange_code` variant-for-variant — the *content* differs slightly (the web engine talks to Microsoft over `fetch`, not `reqwest`) but the *codes* and their translations are identical, so a Dutch user gets the same sign-in error message on desktop and in the browser. New shared `src/lib/apiError.ts::toAppError(e)` passes an already-`AppError`-shaped throw through unchanged and folds anything else (a plain `Error`, a `DOMException`) into `"other"` — used at `webOneDriveSync.ts`'s `login()`/`exchangeCodeDirect()` catch blocks, which used to do their own `e instanceof Error ? e.message : String(e)`.

**Two web-only edge cases handled without inventing web-only codes**: a redirect whose OAuth `state` doesn't match the saved request (no desktop equivalent — desktop's loopback listener has no such cross-request state to mismatch) stays a plain `Other` with its existing English text; a missing PKCE session reuses desktop's own `oneDriveNoPendingSession` code rather than a near-duplicate, since the underlying condition and the fix ("connect again") are the same on both platforms.

**A second real, previously-invisible bug found while wiring this up, same shape as §236's**: `SettingsModal.svelte`'s manual OAuth-paste fallback (`handleOneDriveLogin`/`handleManualAuthSubmit`) had the identical `res.error`-as-plain-string / `err instanceof Error ? err.message : String(err)` pattern already fixed in §236's other call sites — found only because giving `OneDriveLoginResult.error` a real type turned every remaining plain-string read into a `svelte-check` error, which is exactly the enforcement mechanism this whole scheme relies on. Also fixed `boot.ts`'s OAuth-redirect connect flow and `oneDriveSync.ts`'s `signInAgain()`, both already using `describeApiError` from §236 but now getting real per-cause messages instead of always the generic "other" bucket. **A pre-existing gap surfaced but deliberately not fixed here**: `SettingsModal.svelte`'s whole OneDrive/Calendar section (the "Connect Microsoft Account" button, "Connecting…" label, etc.) turns out to have never been translated in Phase 1's component sweep at all — out of scope for this error-code-focused batch, flagged in the design doc for a future pass rather than folded in here.

New keys: 13 in `error.*`, matching the 13 new `AppError` variants one-to-one (10 take a `{ detail }` param carrying the underlying network/OS/Microsoft-response text, exactly Phase 1's "translated headline, untranslated diagnostic tail" shape; 3 are fixed sentences with no dynamic content at all).

Verification: `cargo test` 160/160 (+2, `error.rs`'s serialization tests for a representative unit variant and a `detail`-carrying one); `svelte-check` 264/0; Vitest 589/589 (two real fixes needed along the way — `dictionaries.test.ts` was missing sample params for the 9 new parameterized keys, and `webOneDriveSync.test.ts`'s state-mismatch test asserted `res.error` as a plain string, now a real `AppError` object); full Playwright suite 400/400 clean.

## 238. Multilanguage support, Phase 2 finishes: `FolderSwitchResult`'s composed sentence becomes structured data, with real pluralization

Marien: "Go ahead with both" (this and §239). Closes out §236's other deferred item: `prepare_folder_switch`'s "Couldn't sync {folder} before switching ({why}). Nothing was changed." — the last remaining pre-composed English sentence in the OneDrive sync surface, and the one with real pluralization at stake (`{held} note(s) have a sync conflict to resolve first`).

**New `onedrive::FolderSwitchBlocked`**, a `#[serde(tag = "reason")]` enum with two variants — `SyncFailed { folder_path, detail: Option<AppError> }` and `HeldConflicts { folder_path, count }` — replacing `FolderSwitchResult.message: Option<String>` with `blocked: Option<FolderSwitchBlocked>`. Rust no longer composes any English text here at all: `sync.rs::prepare_folder_switch` just reports *which* of the two blocking conditions applies and the raw data behind it, matching exactly the two branches the original `if !result.success { .. } else { .. }` already had — the frontend does 100% of the translation and composition.

**A genuine serde gotcha caught by the new tests, not assumed**: `#[serde(rename_all = "camelCase")]` on an enum renames the *variant tag values* ("syncFailed"/"heldConflicts") but does **not** cascade into each variant's own field names — `folder_path` serialized as `folder_path`, not `folderPath`, until each variant got its own `#[serde(rename_all = "camelCase")]` too. Every other tagged enum in this codebase (`AppError`) happened to only ever use single-word field names (`detail`), so this bug had no way to surface until now. Three new `mod.rs` unit tests lock in the corrected wire shape.

**New `src/lib/apiError.ts::describeFolderSwitchBlocked(blocked)`** composes and translates the full sentence from the structured data — the held-conflicts count now genuinely pluralizes ("1 note has…" / "N notes have…") in all three languages, something the old `format!("{held} note(s) have...")` could never do correctly in Dutch or German. **The web app's own duplicate implementation in `webOneDriveSync.ts` was rewritten to match** — it used to hand-compose the identical English sentence independently; now it constructs the same `FolderSwitchBlocked` shape Rust does (reusing the *shared*, ts-rs-generated type — its own local `FolderSwitchResult` interface duplicate was deleted in favor of importing the real one from `../types`) and lets the same frontend function translate it. Both `OneDriveFolderPickerModal.svelte` (the feature's only live consumer — gated to `backendKind === "web"`) and two `webBackend.test.ts` tests that asserted on the old composed-string text were updated to check the structured `blocked` shape instead.

Verification: `cargo test` 163/163 (+3, `FolderSwitchBlocked`'s serialization shape — `prepare_folder_switch` itself still isn't unit-tested, same reason as `auth::exchange_code`: it calls `sync_now`, which makes a real network request with no HTTP-mocking layer in this codebase); `svelte-check` 264/0; Vitest 589/589 (two `webBackend.test.ts` assertions updated); full Playwright suite 400/400 clean.

## 239. Multilanguage support: the rest of Settings — OneDrive sign-in, Calendar, Notes Location, Data, and Updates tabs

The other half of "Go ahead with both." §219's original Settings batch only ever translated the Appearance tab's theme/language controls (plus, it turns out, not even the theme options or the tab labels themselves) — everything else in this large component was still hardcoded English: the whole OneDrive Cloud Sync UI (sign-in, manual auth-code paste, Advanced tenant overrides), Calendar, Notes Location, Data (export/import), and Updates. Found while wiring §237's `describeApiError` into `SettingsModal.svelte`'s auth-error handling and noticing how little of the surrounding UI was actually translated — flagged then, done now.

**~100 new keys**, organized under `settings.appearance.*` (glyphs, pure black — genuinely missed the first time), `settings.editor.*`, `settings.calendar.*`, `settings.oneDrive.*` (by far the largest group — sign-in flow, manual code entry, Advanced tenant/client-ID overrides), `settings.notesLocation.*`, `settings.data.*`, plus `settings.tabs.*` for the three tab labels themselves (also missed originally) and two new `common.*`/`about.*` entries.

**Heavy reuse where the exact same state already had a translation**: the Updates tab mirrors `AboutModal`'s own update-status block field for field (§64/#64 — both read the same stores, so they must always agree) — reused `about.updates.sectionLabel`, `about.checkingHint`, `about.isAvailable`, `about.whatsChanged`, `about.downloading.*`, `about.ready.*`, `about.error.*`, and `about.checkNow`, rather than declaring a parallel `settings.updates.*` set that could drift out of sync with About's own wording. Only two settings-only strings (the "check on start" toggle + its hint) needed new keys. `common.cancel`/a new `common.browse` cover the "Cancel"/"Browse…" buttons repeated across Data, Notes Location, and OneDrive. `statusBar.oneDrive.signInAgain`/`.syncingText` are reused for the identical "Sign in again"/"Syncing…" states in the OneDrive section.

**A real gap found in `AboutModal.svelte` itself while reusing its keys**: its "Download & install" button was *still* hardcoded English, missed by §225's original sweep of that same modal — added `about.downloadAndInstall` and fixed it there too, so both surfaces now share the one key.

**A second real, previously-invisible inconsistency fixed along the way**: Settings' own "up to date" wording ("You're up to date.") differed slightly from About's ("You're running the latest version.") for the identical state — reused About's phrasing rather than keeping two near-identical English strings that would only drift further apart once translated into three languages each.

**Split-around-markup handled twice for genuinely static (not dynamic) embedded content**: the two `.agenda.json`-mentioning hints keep the literal `<code>.agenda.json</code>` tag in the template, split into `.before`/`.after` translation halves around it — the file name itself is never translated (same reasoning as every other literal-token/filename case in this sweep), and no `{@html}` is needed since the markup lives in the Svelte template, not the translated string.

**A real Playwright regression, caught by the full suite, not assumed clean**: translating the Settings tab labels (`settings.tabs.appearance`, etc.) made the "Appearance" tab's own label collide with its content's `settings.appearance.sectionLabel` — both now render the identical translated word ("Weergave" in Dutch, "Darstellung" in German), which `i18n-locales.spec.ts` had been relying on being two *different* strings (the tab was hardcoded English, the content was translated) to locate the section unambiguously. Fixed by scoping those two assertions to `.settings-section-label` specifically rather than reverting the (correct) tab-label translation.

Verification: `svelte-check` 264/0; Vitest 589/589 (+2 sample params — `settings.oneDrive.connectedAs.after`, `settings.data.noteCountInFile`); full Playwright suite 400/400 clean after fixing the two `i18n-locales.spec.ts` assertions above; also verified live in the browser under the Dutch system locale (Appearance, Calendar/Notes Location/Data, and Updates tabs all render correctly, including the mid-sentence `.agenda.json` split and the pluralized "up to date" reuse) — the OneDrive Cloud Sync section itself (web-only) wasn't reachable in this manual pass but is covered by `webBackend.test.ts` and the full Playwright suite.

**Phase 2 is now feature-complete for the OneDrive/agenda/sign-in error surface, and the Settings modal is fully translated.** What's left before any of this can ship: the `nl.ts`/`de.ts` native-speaker review pass (still not done by me — flagged throughout, needs Marien for Dutch and a separate reviewer for German) and Phase 3 (website translation, not started, not designed).

## 240. Multilanguage support: the two Shortcuts drawer strings §220 deferred, and Pure Black moved next to Theme

Marien: "On Shortcut and Symbols drawer the text under section headers is not yet translated. Also the text starting with Delegated is not yet translated." Both were real, deliberately deferred gaps — §220's own comment explicitly punted them, since both weave `<kbd>` examples into the *middle* of a sentence (not just a trailing example like "Numbered list," which already had a translation) — the two options on the table then were `{@html}` (unused anywhere in this codebase, a real security-posture call) or a richer per-key schema. Resolved with the latter: each sentence splits into `.part1`/`.part2`/... keys around every literal `<kbd>`, the same "split around markup" shape already used for Settings' `.agenda.json` hints. `<kbd>@name</kbd>`, `<kbd>=&gt;</kbd>`, `<kbd>(@ana, @ben)</kbd>`, `<kbd>=</kbd>`, and `<kbd>====</kbd>` all stay literal token/example syntax outside any translation, per the established convention; "Weekly Sync" — a genuine illustrative example, not a token — got its own `exampleTitle` key so it reads naturally as "Wekelijkse sync" / "Wöchentlicher Sync" instead of staying English mid-sentence.

**Also**: moved "Pure black (OLED)" (Settings → Appearance) to sit directly after the Theme control instead of after Language and Glyphs — Marien: "it is linked to it" (the toggle only applies in dark mode, and is gated on `isDarkResolved`, which is itself derived from `$themeMode`). Pure layout reorder, no logic change.

Verified live in the browser under the Dutch system locale: both Shortcuts drawer sentences now render fully in Dutch with no English fragments, and Pure Black appears directly beneath the Theme hint in Settings' Appearance tab, before Language.

**A verification-process lesson worth recording**: a full Playwright run reported an implausible "67 passed" with garbled output partway through this batch — caused by running a second `npx playwright test` invocation in the foreground while an earlier one was still running in the background, both against the same shared dev server. Not a real regression; a clean, non-concurrent re-run confirmed 400/400. Don't run two Playwright invocations concurrently against the same dev server.

Verification: `svelte-check` 264/0; Vitest 589/589 (no new sample params — all 8 new keys are unparameterized); full Playwright suite 400/400 clean (on a clean, non-concurrent run).

## 241. Multilanguage support: a real layout bug from longer translated text — the "Esc <close>" footer hint wrapping under its own `<kbd>`

Marien: "In Dutch and German the text associated to Esc in the Action Drawer and Section History Drawer is longer than English, leading to the text being deployed under the Esc instead of next to it." A genuine layout regression from translation, not a translation-content bug — "Sluiten"/"Schließen" (vs. English "Close") pushed the trailing `<kbd>Esc</kbd> {close}` hint just past the width `.modal-footer`'s flex layout was leaving it, wrapping the close label onto its own line beneath the key.

**Fixed once, generally, in `app.css`** rather than per-component: `.modal-footer > div:last-child { white-space: nowrap; flex-shrink: 0; }`. This is a shared structural pattern across four modals (Action Drawer, Section History, Command Palette, Search) — all lay out a `.modal-footer` as two flex children, a longer left-hand hint list and a short right-hand "Esc <close>" — so one CSS rule covers all four rather than patching each modal's own markup. The left-hand hint list is deliberately left free to wrap onto a second line if it ever needs to (it already has more visual room to do so safely); pinning only the short trailing hint at its natural width is what stops the wrap Marien saw.

**Verified rigorously, not just eyeballed**: measured the closing `<div>`'s rendered height with and without the fix (toggling `white-space`/`flex-shrink` live via the console) at the same window size that reproduced the bug in the running Dutch-locale app — 12.7px (one line) with the fix in place, 25.3px (two lines) with it removed, confirming the rule is what's actually holding "Esc Sluiten" together rather than the window simply already being wide enough.

Verification: `svelte-check` 264/0 (pure CSS, no `.ts`/`.svelte` logic touched); full Playwright suite 400/400 clean; live-verified in the browser under the Dutch system locale.

## 242. Multilanguage support: the third and final deferred rich-text case — `ConflictModal.svelte`

Marien asked what the third deferred rich-text case was (§228 had flagged it alongside the two Shortcuts drawer ones fixed in §240), then asked to fix it too. `ConflictModal.svelte`'s explanatory paragraph — "This note was modified outside ChronoNote... unless you pick *Keep disk* or *Keep mine* without saving a copy." — weaves two `<em>`-wrapped button-name references mid-sentence, the same shape as §220's "Delegated" case.

Split into `.part1`/`.keepDiskRef`/`.part2`/`.keepMineRef`/`.part3` around each literal `<em>`, same pattern as §240. **The two em-wrapped references got their own dedicated keys rather than reusing `conflictModal.keepDiskVersion`/`.keepMyVersion`** (the existing button-label keys): the paragraph's own wording ("Keep disk", "Keep mine") is deliberately shorter than the full button labels ("Keep disk version", "Keep my version") even in the original English — not a literal quote, so forcing a shared key would have meant changing one or the other's wording.

With this fixed, **all three of Phase 1's deferred rich-text cases (§220's two, §228's one) are now translated** — there is no longer any UI text in the app that's deliberately left English for a `{@html}`-avoidance reason. What remains permanently untranslated by design (the literal token vocabulary, search's filter-chip syntax, raw OS/network/Rust diagnostic error tails, language autonyms, and the marketing website) is documented in the design doc's Non-goals section and `error.rs`'s own comment — not gaps, deliberate architecture.

Verification: `svelte-check` 264/0; Vitest 589/589; full Playwright suite 400/400 clean.
