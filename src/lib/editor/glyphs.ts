 ...
 *
 * The bullet token (`- `) uses a lookbehind (`(?<=^\s*)-\s`) rather than
 * capturing the leading indentation in the match itself: indentation must
 * stay untouched, real, visible whitespace (that's how nesting works —
 * spec: two spaces per level), and the lookbehind means the match is
 * already just the `- ` to replace, with no indent-length arithmetic
 * needed to find where it starts. */
const renderMatcher = new MatchDecorator({
  regexp: /(^!\s)|(^#\s)|(^v\s)|(^>\s)|(=>\s@\w+)|(=>\s)|((?<=^\s*)-\s)/gm,
  decorate(add, from, to, match, view) {
    ...
    if (text.startsWith("# ")) {
      add(from, to, Decoration.replace({ widget: new InlineGlyphWidget("☐", "glyph-open") }));
      return;
    }
    if (text.startsWith("v ")) {
      add(from, to, Decoration.replace({ widget: new InlineGlyphWidget("☑", "glyph-done") }));
      return;
    }
    if (text.startsWith("> ")) {
      add(from, to, Decoration.replace({ widget: new InlineGlyphWidget("»", "glyph-progress") }));
      return;
    }
    if (text.startsWith("=> @")) {
    ...
    if (text.startsWith("=>")) {
      add(from, to, Decoration.replace({ widget: new InlineGlyphWidget("➔", "glyph-followup") }));
      return;
    }
 ...
 * (never the `@name` mark, never the `!`-line mark, never a bullet's
 * leading indentation — those stay ordinary, fully editable text). Kept as
 * a separate matcher rather than filtering `renderMatcher`'s mixed set,
 * since `EditorView.atomicRanges` must never include `@name` — that would
 * make it uneditable again, defeating the whole point of marking it
 * instead of replacing it. The actual Decoration value here is never
 * rendered (only fed to `atomicRanges`), so its content doesn't matter,
 * only its range. */
const atomicMatcher = new MatchDecorator({
  regexp: /(^#\s)|(^v\s)|(^>\s)|(=>\s)|((?<=^\s*)-\s)/gm,
...
/** Makes each glyph's underlying raw token (`# `, `v `, `> `, `- `, the
 * `=> ` portion of a follow-up/delegation) an atomic unit for cursor
 * motion and selection: a selection boundary can never land in the middle
 * of a replaced token. Without this, selecting a line for copy could
 * silently drop half of a token, and the glyph wouldn't visually appear
 * selected even when its text was included. It's also what makes
 * backspace remove a whole `=> ` glyph in one step when the cursor sits
 * right after it. */
