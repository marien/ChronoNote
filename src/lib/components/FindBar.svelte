...
<div class="find-bar" role="search">
  ...
  <button type="button" class="find-btn" on:click={() => editorApi?.find.prev()} aria-label="Previous match" title="Previous (Shift+Enter)">‹</button>
  <button type="button" class="find-btn" on:click={() => editorApi?.find.next()} aria-label="Next match" title="Next (Enter)">›</button>
  <button type="button" class="find-btn" on:click={close} aria-label="Close find" title="Close (Esc)">✕</button>
</div>
